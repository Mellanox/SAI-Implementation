...
typedef struct fx_handle {
    ...
    uint8_t              num_sdk_actions_for_action_id[9];
...
fx_table_x_entry_add  add_table_entry_control_in_rif_table_bitmap_classification, add_table_entry_control_in_rif_table_bitmap_router, add_table_entry_control_out_rif_table_l3_vxlan;
fx_table_x_entry_add * fx_table_x_entry_add_fn[3] = {add_table_entry_control_in_rif_table_bitmap_classification, add_table_entry_control_in_rif_table_bitmap_router, add_table_entry_control_out_rif_table_l3_vxlan};
fx_table_x_default_entry_set  fx_control_in_rif_table_bitmap_classification_default_entry_set, fx_control_in_rif_table_bitmap_router_default_entry_set, fx_control_out_rif_table_l3_vxlan_default_entry_set;
fx_table_x_default_entry_set * fx_table_x_default_entry_set_fn[3] = {fx_control_in_rif_table_bitmap_classification_default_entry_set, fx_control_in_rif_table_bitmap_router_default_entry_set, fx_control_out_rif_table_l3_vxlan_default_entry_set};
fx_table_x_init_keys  fx_init_key_list_control_in_rif_table_bitmap_classification, fx_init_key_list_control_in_rif_table_bitmap_router, fx_init_key_list_control_out_rif_table_l3_vxlan;
fx_table_x_init_keys * fx_table_x_init_keys_fn[3] = {fx_init_key_list_control_in_rif_table_bitmap_classification, fx_init_key_list_control_in_rif_table_bitmap_router, fx_init_key_list_control_out_rif_table_l3_vxlan};
fx_table_x_deinit_keys  fx_deinit_key_list_control_in_rif_table_bitmap_classification, fx_deinit_key_list_control_in_rif_table_bitmap_router, fx_deinit_key_list_control_out_rif_table_l3_vxlan;
fx_table_x_deinit_keys * fx_table_x_deinit_keys_fn[3] = {fx_deinit_key_list_control_in_rif_table_bitmap_classification, fx_deinit_key_list_control_in_rif_table_bitmap_router, fx_deinit_key_list_control_out_rif_table_l3_vxlan};
/////////////////////////////
// General functions
/////////////////////////////
...
{
    LOG_VAR_NAME(__MODULE__) = severity;
...
create_acl_p4_key(fx_handle_t handle, struct acl_table * acl_table, struct fx_custom_params *custom)
{
    ...
    if (custom && custom->is_cb) {
        ...
        if (rc) {
            // DEBUG
            SYSLOGF(SX_LOG_DEBUG, "Failed sx_api_acl_custom_bytes_set, acl keys: %s", SX_STATUS_MSG(rc));
            for (uint32_t i = custom->cb_key_index_start; i < custom->cb_key_index_start + custom->cb_key_list_size; i++) {
                SYSLOGF(SX_LOG_DEBUG, "%d, ", acl_table->key_list[i]);
            }
            SYSLOGF(SX_LOG_DEBUG, "\n");
            SYSLOGF(SX_LOG_ERROR, "ERROR: SDK API sx_api_acl_custom_bytes_set failed: [%s]\n", SX_STATUS_MSG(rc));
            return rc;
        }
    }
    ...
    if (rc) {
        // TODO - syslog from SDK is SX_SX_LOG_ERROR("Failed calculating key blocks.\n") if
        // the acl key size is too large. This is hard to debug, need a compiler semantic check.
        if (acl_table->key_handle == 0) {
            SYSLOGF(SX_LOG_ERROR, "ERROR: Failed to set %i %s keys (hw limit exceeded)\n",
                    acl_table->key_count, acl_table->table_name);
        }
        // DEBUG
        SYSLOGF(SX_LOG_DEBUG, "Failed sx_api_acl_flex_key_set: %s %s", SX_STATUS_MSG(rc), acl_table->table_name);
        SYSLOGF(SX_LOG_DEBUG, "handle 0x%x, %d keys: ", acl_table->key_handle, acl_table->key_count);
        SYSLOGF(SX_LOG_DEBUG, "key_list = ");
        for (uint32_t i=0; i < acl_table->key_count; i++) {
            SYSLOGF(SX_LOG_DEBUG, "%d, ", acl_table->key_list[i]);
        }
        SYSLOGF(SX_LOG_DEBUG, "\n");
        SYSLOGF(SX_LOG_ERROR, "ERROR: SDK API sx_api_acl_flex_key_set failed: [%s]\n", SX_STATUS_MSG(rc));
    }
...
sx_status_t create_acl_table(fx_handle_t handle,struct acl_table * acl_table)
{
    sx_status_t rc = sx_api_acl_region_set(g_sdk_handle,
            ...
            &acl_table->region_id);
    if (rc) {
        SYSLOGF(SX_LOG_ERROR, "ERROR: SDK API sx_api_acl_region_set failed: [%s]\n", SX_STATUS_MSG(rc));
        return(rc);
    }
    else {
        SYSLOGF(SX_LOG_DEBUG, "sx_api_acl_region_set to id %#x\n", acl_table->region_id);
    }
    ...
    rc = sx_api_acl_set(g_sdk_handle,
            ...
            &acl_table->acl_id);
    if (rc) {
        SYSLOGF(SX_LOG_ERROR, "Error %d: failed to create acl: [%s]\n", rc, SX_STATUS_MSG(rc));
        return rc;
    }
...
sx_status_t assign_tables_to_group(fx_handle_t handle,sx_acl_direction_t direction, sx_acl_id_t* pipe_id_list, uint32_t pipe_table_num , sx_acl_id_t* pipe_group_id){
  ...
  if (rc != SX_STATUS_SUCCESS) {
      SYSLOGF(SX_LOG_ERROR, "ERROR: SDK API sx_api_acl_group_set failed: [%s]\n", SX_STATUS_MSG(rc));
      return rc;
  }
  ...
  SYSLOGF(SX_LOG_DEBUG, "SDK API: pipe tables added to pipe Group Id: %#x \n", *pipe_group_id);
  return SX_STATUS_SUCCESS;
}
// ----------------- port/ rif bindings -----------------------------
...
fx_get_bindable_port_list(fx_handle_t handle, sx_port_log_id_t *if_list, uint32_t *if_list_cnt)
{
  ...
  *if_list_cnt = 0;
  ...
  SYSLOGF(SX_LOG_DEBUG, "Bindable ports:");
...
sx_status_t fx_get_bindable_rif_list(fx_handle_t handle, sx_router_interface_t *if_list, uint32_t *if_list_cnt)
{
  ...
  if (*if_list_cnt == 0)
  {
    SYSLOGF(SX_LOG_INFO, "WARNING: no RIF configured in switch.\n");
    return SX_STATUS_SUCCESS;
  }
  ...
  while (curr_rif_cnt > 0) {
    ...
    if (rc != SX_STATUS_SUCCESS) {
      SYSLOGF(SX_LOG_ERROR, "Error: Iteration #%d SDK API sx_api_router_interface_iter_get with curr rif %d failed: [%s]\n",
             curr_rif_idx-1, if_list[curr_rif_idx-1], SX_STATUS_MSG(rc));
      return rc;
    }
  }
  if (print_rifs) {
    SYSLOGF(SX_LOG_DEBUG, "Bindable rifs:");
    for (uint32_t i = 0; i < *(if_list_cnt); i++)
    {
      SYSLOGF(SX_LOG_DEBUG, " 0x%x", if_list[i]);
    }
...
port_bind(fx_handle_t handle, const sx_acl_id_t group_id,boolean_t bind, sx_port_log_id_t *if_list, uint32_t if_list_cnt)
{
  ...
  for(uint32_t i=0;i<if_list_cnt;i++) {
    ...
    sx_status_t rc = sx_api_acl_port_bind_set(g_sdk_handle,
                                            ...
                                            group_id);
    if (rc != SX_STATUS_SUCCESS) {
      SYSLOGF(SX_LOG_ERROR, "ERROR: SDK API sx_api_acl_port_bind_set failed: [%s] 0x%x\n", SX_STATUS_MSG(rc), if_list[i]);
...
rif_bind(fx_handle_t handle, const sx_acl_id_t group_id,boolean_t bind, sx_router_interface_t *if_list, uint32_t if_list_cnt){
  ...
  for(uint32_t i=0;i<if_list_cnt;i++) {
    ...
    if (rc != SX_STATUS_SUCCESS) {
        SYSLOGF(SX_LOG_ERROR, "ERROR: SDK API sx_api_acl_rif_bind_set failed: [%s] %x\n", SX_STATUS_MSG(rc), if_list[i]);
        if (rc > worst_rc) worst_rc = rc;
    }
  }
  SYSLOGF(SX_LOG_DEBUG, ".\n");
...
delete_acl_group(fx_handle_t handle,sx_acl_direction_t direction, sx_acl_id_t* pipe_group_id){
    sx_status_t rc = sx_api_acl_group_set(g_sdk_handle,
                                     ...
                                     pipe_group_id);
    if (rc != SX_STATUS_SUCCESS) {
        SYSLOGF(SX_LOG_ERROR, "ERROR: SDK API sx_api_group_delete failed: [%s]\n", SX_STATUS_MSG(rc));
    }
    else {
        SYSLOGF(SX_LOG_DEBUG, "SDK API: group destroyed \n");
    }
    return rc;
}
...
{
  SYSLOGF(SX_LOG_DEBUG, "delete P4 table\n");
...
//  acl_table->rule_counters = 0;
  ...
  rc2=sx_api_acl_region_set(g_sdk_handle,
       SX_ACCESS_CMD_DESTROY,
            ...
            &acl_table->region_id);
  if(rc2) {
    SYSLOGF(SX_LOG_ERROR, "ERROR: failed to delete table region: [%s]  \n",SX_STATUS_MSG(rc2));
   }
   return rc1 ? rc1 : rc2;
}
...
delete_p4_key(fx_handle_t handle, struct acl_table *acl_table, struct fx_custom_params *custom)
{
  ...
  sx_status_t rc1 = sx_api_acl_flex_key_set(g_sdk_handle,
               ...
               &acl_table->key_handle);
  if(rc1) {
    SYSLOGF(SX_LOG_ERROR, "ERROR: fail to Delete P4 Table key type. [%s]\n", SX_STATUS_MSG(rc1));
  }
  SYSLOGF(SX_LOG_DEBUG, "delete CB\n");
  sx_status_t rc2 = SX_STATUS_SUCCESS;
  if (custom && custom->is_cb) {
      /* P4C statically decides if the ACL key needs custom bytes */
...
sx_status_t get_table_index_from_id (fx_handle_t handle, const fx_table_id_t table_id, int *table_index){
  ...
  SYSLOGF(SX_LOG_ERROR, "ERROR: Flex Table not initialized, ID: %i\n", table_id);
  ...
  for(uint32_t i=0;i<table->table_size;i++){
    if (get_bitmap(table->valid_offsets, i) == 1) {
      ...
      if(rc){
        ...
        SYSLOGF(SX_LOG_DEBUG, "*rule %d; counter byte %" PRIu64 "; packets %" PRIu64 ";\n",i,counter.flow_counter_bytes,counter.flow_counter_packets);
      }
    }
  }
  SYSLOGF(SX_LOG_DEBUG, "\n");
...
sx_status_t fx_table_rule_counters_clear_all(fx_handle_t handle, fx_table_id_t table_id){
  ...
  SYSLOGF(SX_LOG_DEBUG, "Counters cleared\n");
  return rc_final;
}
...
sx_status_t create_p4_table(fx_handle_t handle, struct acl_table *table, struct fx_custom_params *custom) {
  SYSLOGF(SX_LOG_DEBUG, "FLEX-BASE: create_p4_table. creating key\n");
...
void get_exact_offset_internal(struct acl_table *table, sx_acl_rule_offset_t *offset_ptr) {
    ...
    SYSLOGF(SX_LOG_ERROR, "exact match table %s is full\n", table->table_name);
    return;
}
...
int add_acl_rule_internal(fx_handle_t handle, sx_flex_acl_flex_rule_t* rule, sx_acl_rule_offset_t* rule_offset, struct acl_table *table) {
  ...
  if(rc) {
    ...
    if (rule->key_desc_list_p) {
        SYSLOGF(SX_LOG_DEBUG, "cb rule key : %d\n", rule->key_desc_list_p->key.custom_byte);
        SYSLOGF(SX_LOG_DEBUG, "first rule key id : %d\n", rule->key_desc_list_p->key_id);
        SYSLOGF(SX_LOG_DEBUG, "cb rule mask : %d\n", rule->key_desc_list_p->mask.custom_byte);
    }
    SYSLOGF(SX_LOG_DEBUG, "region id : %#x\n", table->region_id);
    SYSLOGF(SX_LOG_DEBUG, "acl id : %#x\n", table->acl_id);
    SYSLOGF(SX_LOG_DEBUG, "group id : %#x\n", table->group_id);
    for (uint32_t i=0; i < rule->key_desc_count; i++) {
        SYSLOGF(SX_LOG_DEBUG, "\tkey_list[%d] = %d  ", i, rule->key_desc_list_p[i].key_id);
        if (rule->key_desc_list_p[i].key_id == FLEX_ACL_KEY_DIP ||
            rule->key_desc_list_p[i].key_id == FLEX_ACL_KEY_SIP) {
            SYSLOGF(SX_LOG_DEBUG, "\tkey %s  ", inet_ntoa(rule->key_desc_list_p[i].key.dip.addr.ipv4));
            SYSLOGF(SX_LOG_DEBUG, "\tmask %s\n", inet_ntoa(rule->key_desc_list_p[i].mask.dip.addr.ipv4));
        }
        else {
            SYSLOGF(SX_LOG_DEBUG, "\tkey 0x%x  \tmask 0x%x\n",
                (uint32_t)rule->key_desc_list_p[i].key.src_port,(uint32_t)rule->key_desc_list_p[i].mask.vni_key);
        }
    }
  }
  if(rc) {
    SYSLOGF(SX_LOG_ERROR, "ERROR %d: [%s] Failed to insert rule at offset %d\n", rc, SX_STATUS_MSG(rc),*rule_offset);
    return -1;
  } else {
      SYSLOGF(SX_LOG_DEBUG, "Inserted rule at offset %d\n", *rule_offset);
  }
...
sx_status_t init_port_mapping(fx_handle_t handle) {
    ...
    sx_status_t rc = sx_api_port_swid_port_list_get(g_sdk_handle,
            ...
            &handle->port_cnt);
    if (rc) {
        SYSLOGF(SX_LOG_ERROR, "Error sx_api_port_swid_port_list_get: %s\n", SX_STATUS_MSG(rc));
        return rc;
    }
    ...
    for (unsigned int i = 0; i < handle->port_cnt; i++) {
        ...
        if (i < pattr) {
            ...
            if (label <= MAX_LOG_PORTS) { // index zero based, label 1 based
                ...
                SYSLOGF(SX_LOG_ERROR, "Unable to map Logical port %i : %#010x,  label: %i\n", i, handle->log_port_list[i], label);
            }
        }
        else {
            SYSLOGF(SX_LOG_DEBUG, "Logical port %i : %#010x,  label: UNKNOWN \n", i, handle->log_port_list[i]);
        }
    }
    free(port_attrs);
    return rc;
...
sx_status_t fx_logical_port_get(fx_handle_t handle, uint32_t label_port, sx_port_log_id_t* log_port) {
    sx_status_t rc = SX_STATUS_SUCCESS;
    if (handle == NULL) {
        SYSLOGF(SX_LOG_ERROR, "fx_logical_port_get() %s\n","invalid handle parameter");
        return SX_STATUS_PARAM_ERROR;
    }
...
sx_status_t fx_table_entry_remove(fx_handle_t handle, const fx_table_id_t table_id, sx_acl_rule_offset_t offset){
  ...
  if (table->range_table==NULL){
    ...
    switch(table_id){
      default:
        SYSLOGF(SX_LOG_ERROR, "ERROR: fx_table_entry_remove table_id: %d, table_id missmatch.\n",table_id);
        return(SX_STATUS_PARAM_ERROR);
    }
  }
}

sx_status_t fx_bytearray_data_put(fx_bytearray_t *out, const void *in, size_t size)
{
    if (!out) return SX_STATUS_PARAM_NULL;
    if (!in) return SX_STATUS_PARAM_NULL;

    if (out->len < size) return SX_STATUS_PARAM_ERROR;
...
sx_status_t fx_table_entry_get(fx_handle_t handle,
                               const fx_table_id_t table_id,
                               sx_acl_rule_offset_t offset,
                               fx_action_id_t *action_id,
                               fx_key_list_t *keys,
                               fx_param_list_t *params)
{
    uint32_t fx_key_idx = 0, fx_param_idx = 0;
    const sx_flex_acl_flex_action_t *sx_action;

    if (!action_id) return SX_STATUS_PARAM_NULL;
    if (!keys) return SX_STATUS_PARAM_NULL;
    if (!params) return SX_STATUS_PARAM_NULL;
    ...
    const struct acl_table *table = &handle->acl_tables[table_index];

    sx_flex_acl_flex_rule_t rule;
    rc = sx_lib_flex_acl_rule_init(table->key_handle, 10, &rule);
    if (rc) return rc;
    ...
    for (uint32_t i = 0; i < rule.key_desc_count; i++) {
        ...
        switch (rule.key_desc_list_p[i].key_id) {
        case FLEX_ACL_KEY_IRIF:
            data = &rule.key_desc_list_p[i].key.irif;
            mask = &rule.key_desc_list_p[i].mask.irif;
            data_size = sizeof(rule.key_desc_list_p[i].key.irif);
            mask_size = sizeof(rule.key_desc_list_p[i].mask.irif);
            break;

        case FLEX_ACL_KEY_DMAC:
            data = &rule.key_desc_list_p[i].key.dmac.ether_addr_octet;
            mask = &rule.key_desc_list_p[i].mask.dmac.ether_addr_octet;
            data_size = sizeof(uint32_t);
            mask_size = sizeof(uint32_t);
            break;

        case FLEX_ACL_KEY_DIP:
            data = &rule.key_desc_list_p[i].key.dip.addr.ipv4.s_addr;
            mask = &rule.key_desc_list_p[i].mask.dip.addr.ipv4.s_addr;
            data_size = sizeof(rule.key_desc_list_p[i].key.dip.addr.ipv4.s_addr);
            mask_size = sizeof(rule.key_desc_list_p[i].mask.dip.addr.ipv4.s_addr);
            break;

        case FLEX_ACL_KEY_USER_TOKEN:
            data = &rule.key_desc_list_p[i].key.user_token;
            mask = &rule.key_desc_list_p[i].mask.user_token;
            data_size = sizeof(rule.key_desc_list_p[i].key.user_token);
            mask_size = sizeof(rule.key_desc_list_p[i].mask.user_token);
            break;
        ...
        rc = fx_bytearray_data_put(&keys->keys[fx_key_idx].key, data, data_size);
        if (rc) goto out;

        rc = fx_bytearray_data_put(&keys->keys[fx_key_idx].mask, mask, mask_size);
        if (rc) goto out;
    ...
    if (params->len < rule.action_count) {
        rc = SX_STATUS_PARAM_ERROR;
        goto out;
    }
    ...
    *action_id = NOACTION_ID;
    for (uint32_t i = 0; i < rule.action_count; i++) {
        sx_action = &rule.action_list_p[i];
        switch (sx_action->type) {
        case SX_FLEX_ACL_ACTION_FORWARD:
            if (sx_action->fields.action_forward.action != SX_ACL_TRAP_FORWARD_ACTION_TYPE_DISCARD) {
                return SX_STATUS_ERROR;
            }
        ...
        case SX_FLEX_ACL_ACTION_SET_DST_MAC:
            rc = fx_bytearray_data_put(&params->params[fx_param_idx],
                    &sx_action->fields.action_set_dst_mac.mac, sizeof(uint32_t));
            if (rc) goto out;
        ...
        case SX_FLEX_ACL_ACTION_TRAP:
            rc = fx_bytearray_data_put(&params->params[fx_param_idx],
                    &sx_action->fields.action_trap.trap_id,
                    sizeof(sx_action->fields.action_trap.trap_id));
            if (rc) goto out;
        ...
        case SX_FLEX_ACL_ACTION_UC_ROUTE:
            if (sx_action->fields.action_uc_route.uc_route_type == SX_UC_ROUTE_TYPE_NEXT_HOP) {
                rc = fx_bytearray_data_put(&params->params[fx_param_idx],
                        &sx_action->fields.action_uc_route.uc_route_param.ecmp_id,
                        sizeof(sx_action->fields.action_uc_route.uc_route_param.ecmp_id));
                *action_id = CONTROL_IN_RIF_TO_NEXTHOP_ID;
                fx_param_idx++;
            }
            else if (sx_action->fields.action_uc_route.uc_route_type == SX_UC_ROUTE_TYPE_LOCAL) {
                rc = fx_bytearray_data_put(&params->params[fx_param_idx],
                        &sx_action->fields.action_uc_route.uc_route_param.local_egress_rif,
                        sizeof(sx_action->fields.action_uc_route.uc_route_param.local_egress_rif));
        ...
        case SX_FLEX_ACL_ACTION_NVE_TUNNEL_ENCAP:
            rc = fx_bytearray_data_put(&params->params[fx_param_idx],
                    ...
                    sizeof(sx_action->fields.action_nve_tunnel_encap.tunnel_id));
            fx_param_idx++;

            rc = fx_bytearray_data_put(&params->params[fx_param_idx],
                    &sx_action->fields.action_nve_tunnel_encap.underlay_dip.addr,
                    sizeof(sx_action->fields.action_nve_tunnel_encap.underlay_dip.addr));
            fx_param_idx++;
        ...
        case SX_FLEX_ACL_ACTION_SET_USER_TOKEN:
            rc = fx_bytearray_data_put(&params->params[fx_param_idx],
                    &sx_action->fields.action_set_user_token.user_token,
                    sizeof(sx_action->fields.action_set_user_token.user_token));
            fx_param_idx++;
            break;
...
sx_status_t fx_table_entry_offset_find(fx_handle_t handle, const fx_table_id_t table_id, fx_key_list_t keys,
    ...
    for (uint32_t i=0; i<rules_count; i++) {
        rc = sx_lib_flex_acl_rule_init(table->key_handle, 10, &rules[i]);
        if (rc) {
            for (uint32_t j = 0; j<i; j++) {
                sx_lib_flex_acl_rule_deinit(&rules[j]);
            }
            free(rules);
            return rc;
        }
    ...
    if (table->range_table==NULL){
                ...
                if (rules[i].key_desc_count <= keys.len) {
                    if (rules[i].key_desc_list_p != NULL) {
                        for (uint32_t j=0; j<rules[i].key_desc_count; j++) {
                            SYSLOGF(SX_LOG_DEBUG, "Rule %i: key id %u\n",i, rules[i].key_desc_list_p[j].key_id);
                            const sx_acl_key_fields_t *key_fields = &rules[i].key_desc_list_p[j].key;
                            // fx_byte array has null values for key index that mean don't care
                            int key_count = 0;
                        ...
                        if (found) {
                            *offset = offsets[i];
                            rc = SX_STATUS_SUCCESS;
                            SYSLOGF(SX_LOG_DEBUG, "FOUND Rule %i: offset %i\n", i, *offset);
                            break;
                        }
                    }
                }
                else {
                    SYSLOGF(SX_LOG_ERROR, "Mismatch error in key count (%i > %i), table ID %i\n",
                            rules[i].key_desc_count, (int)keys.len, table_id);
                    for (uint32_t x=0; x < rules[i].key_desc_count; x++) {
                        if (rules[i].key_desc_list_p != NULL)
                            SYSLOGF(SX_LOG_ERROR,"\t Key ID %i, value %#010x\n", rules[i].key_desc_list_p[x].key_id, rules[i].key_desc_list_p[x].key.mpls_label_id_1);
                    }
                    for (int y=0; y < keys.len; y++) {
                        if (keys.keys[y].key.data != NULL)
                            SYSLOGF(SX_LOG_ERROR,"\t Key #%i, data %#010x\n", y, *((uint32_t*)keys.keys[y].key.data));
                    }
                    rc = SX_STATUS_PARAM_ERROR;
                }
            }
            if (!found) {
                SYSLOGF(SX_LOG_ERROR,"Failed to find modify key out of %i flex rules\n", rules_count);
            }
        }
    }
    else {
        ...
        rc = SX_STATUS_PARAM_ERROR;
        SYSLOGF(SX_LOG_ERROR,"Modify range table not supported: %i\n", rc);
    }
    for (uint32_t i=0; i<rules_count; i++) {
...
void fx_table_deinit_keys(fx_handle_t handle, fx_table_id_t table_id, fx_key_list_t keys[], size_t rule_cnt) {
  ...
  sx_status_t rc = get_table_index_from_id (handle, table_id, &table_index);
  if (rc) {
      SYSLOGF(SX_LOG_ERROR, "Error getting table index from id %d [fx_table_init_keys]\n", table_id);
      return;
  }
...
sx_status_t check_action_params(fx_param_list_t params, fx_action_id_t action_id) {
  /* CHECK ACTION PARAMS */
	switch(action_id){
		...
		{
          if (params.len != 1) {
            SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got invalid parameters list of length %zu while expecting 1 in action control_in_rif.set_metadata\n", params.len);
            return SX_STATUS_PARAM_ERROR;
          }
                break;
		}
		...
		{
          if (params.len != 1) {
            SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got invalid parameters list of length %zu while expecting 1 in action control_in_rif.to_cpu\n", params.len);
            return SX_STATUS_PARAM_ERROR;
          }
                break;
		}
		...
		{
          if (params.len != 2) {
            SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got invalid parameters list of length %zu while expecting 2 in action control_in_rif.to_nexthop\n", params.len);
            return SX_STATUS_PARAM_ERROR;
          }
                break;
		}
		...
		{
          if (params.len != 2) {
            SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got invalid parameters list of length %zu while expecting 2 in action control_out_rif.tunnel_encap\n", params.len);
            return SX_STATUS_PARAM_ERROR;
          }
                break;
		}
		...
		{
          if (params.len != 1) {
            SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got invalid parameters list of length %zu while expecting 1 in action control_in_rif.to_local\n", params.len);
            return SX_STATUS_PARAM_ERROR;
          }
                break;
		}
...
void set_action_list(fx_handle_t handle, int action_id, fx_param_list_t params, struct acl_table *table, sx_acl_rule_offset_t* offset_ptr , sx_flex_acl_flex_rule_t* rule ) {
    if (rule->action_list_p == NULL) { SYSLOGF(SX_LOG_ERROR, "Flex action uninitialized (sx_lib_flex_acl_rule_init)\n"); return; }
	switch(action_id) { // 7 unique actions out of 9 action invocations
		...
		case CONTROL_IN_RIF_SET_METADATA_ID:
		{
            ...
            if (4 > params.params[0].len) { SYSLOGF(SX_LOG_ERROR, "ERROR in action parameter size\n"); }
		...
		case CONTROL_IN_RIF_TO_NEXTHOP_ID:
		{
			/* set_meta_reg(tunnel_index, 0xfff) */
			rule->action_list_p[0] = (sx_flex_acl_flex_action_t){
				.type =  SX_FLEX_ACL_ACTION_SET_USER_TOKEN,
			/*	.fields.action_set_user_token.user_token = *(uint16_t*)(params.params[1].data), */
				.fields.action_set_user_token.mask = (uint16_t) 4095,
			};
			memcpy(&rule->action_list_p[0].fields.action_set_user_token.user_token, params.params[0].data, params.params[0].len);
		...
		case CONTROL_OUT_RIF_TUNNEL_ENCAP_ID:
		{
			...
			/*	.fields.action_nve_tunnel_encap.tunnel_id = *(sx_tunnel_id_t*)(params.params[0].data), */
			/*	.fields.action_nve_tunnel_encap.underlay_dip.addr = *(sx_ip_v4_addr_t*)(params.params[1].data), */
				.fields.action_nve_tunnel_encap.underlay_dip.version = (sx_ip_version_t) SX_IP_VERSION_IPV4,
			};
			memcpy(&rule->action_list_p[0].fields.action_nve_tunnel_encap.tunnel_id, params.params[0].data, params.params[0].len);
			memcpy(&rule->action_list_p[0].fields.action_nve_tunnel_encap.underlay_dip.addr, params.params[1].data, params.params[1].len);
			/* set_meta_reg(0x0, 0xfff) */
			rule->action_list_p[1] = (sx_flex_acl_flex_action_t){
				.type =  SX_FLEX_ACL_ACTION_SET_USER_TOKEN,
				.fields.action_set_user_token.user_token = (uint16_t) 0,
				.fields.action_set_user_token.mask = (uint16_t) 4095,
			};
			/* hit_counter() */
			/*rule->action_list_p[2] = (sx_flex_acl_flex_action_t){
		...
		default:
            SYSLOGF(SX_LOG_ERROR, "ERROR: requested action is not in the action list %d\n", action_id);
	}
	return;
}
...
//  }
  ...
  if (rc) {
      SYSLOGF(SX_LOG_ERROR, "Error: create_control_in_rif_table_bitmap_classification, table #%d with acl ID %d\n", table_index, table->acl_id);
     return rc;
  } else {
      SYSLOGF(SX_LOG_DEBUG, "create_control_in_rif_table_bitmap_classification, table #%d with acl ID %d\n", table_index, table->acl_id);
  }
...
sx_status_t delete_control_in_rif_table_bitmap_classification(fx_handle_t handle) {
  SYSLOGF(SX_LOG_DEBUG, "delete_control_in_rif_table_bitmap_classification. \n");
  const int table_index = 0; // 0 is replaced by macro
  struct acl_table *table = &handle->acl_tables[table_index];
  struct fx_custom_params custom = {false, 0, 0, 0, 0};
  fill_custom_bytes_control_in_rif_table_bitmap_classification(&custom);
...
sx_status_t fx_control_in_rif_table_bitmap_classification_default_entry_set(fx_handle_t handle, struct acl_table *table, fx_param_list_t params,int action_id){
  ...
  if (rc) {return rc;}
  rule->valid=1;
  SYSLOGF(SX_LOG_DEBUG, "insert rule to control_in_rif_table_bitmap_classification index %d\n", offset);
...
sx_status_t check_input_lengths_control_in_rif_table_bitmap_classification(fx_key_list_t keys, fx_param_list_t params, fx_action_id_t action_id) {
  ...
  if (keys.len != 1) {
    SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got invalid keys length %zu while expecting 1, in table control_in_rif.table_bitmap_classification\n", keys.len);
    return SX_STATUS_PARAM_ERROR;
  }
  ...
  if (keys.keys[0].key.data != NULL) {
    if (keys.keys[0].key.len != sizeof(sx_router_interface_t)) {
      SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got key (0) of length %zu while expecting %lu, in table control_in_rif.table_bitmap_classification\n", keys.keys[0].key.len, sizeof(sx_router_interface_t));
      return SX_STATUS_PARAM_ERROR;
    }
  }
...
sx_status_t remove_table_entry_control_in_rif_table_bitmap_classification(fx_handle_t handle, sx_acl_rule_offset_t offset){
  const int table_index = 0; // 0 is replaced by macro
  struct acl_table *table = &handle->acl_tables[table_index];
  SYSLOGF(SX_LOG_DEBUG, "remove rule from control_in_rif_table_bitmap_classification, offset =  %d\n", offset);
  return remove_table_entry(handle,table, offset);
}
...
//  }
  ...
  sx_status_t rc =  create_p4_table(handle, table, &custom);
  if (rc) {
      SYSLOGF(SX_LOG_ERROR, "Error: create_control_in_rif_table_bitmap_router, table #%d with acl ID %d\n", table_index, table->acl_id);
     return rc;
  } else {
      SYSLOGF(SX_LOG_DEBUG, "create_control_in_rif_table_bitmap_router, table #%d with acl ID %d\n", table_index, table->acl_id);
  }
	table->range_table = NULL;  
  pipe_id_list[pipe_ind] = table->acl_id;
  return SX_STATUS_SUCCESS;
...
sx_status_t delete_control_in_rif_table_bitmap_router(fx_handle_t handle) {
  SYSLOGF(SX_LOG_DEBUG, "delete_control_in_rif_table_bitmap_router. \n");
  const int table_index = 1; // 1 is replaced by macro
  struct acl_table *table = &handle->acl_tables[table_index];
  struct fx_custom_params custom = {false, 0, 0, 0, 0};
  fill_custom_bytes_control_in_rif_table_bitmap_router(&custom);
...
sx_status_t fx_control_in_rif_table_bitmap_router_default_entry_set(fx_handle_t handle, struct acl_table *table, fx_param_list_t params,int action_id){
  ...
  if (rc) {return rc;}
  rule->valid=1;
  SYSLOGF(SX_LOG_DEBUG, "insert rule to control_in_rif_table_bitmap_router index %d\n", offset);
  /* __TABLE_DEFAULT_ENTRY_KEYS_||_ */
  rule->key_desc_list_p = NULL;
  rule->key_desc_count = 0;
  // init action
...
sx_status_t check_input_lengths_control_in_rif_table_bitmap_router(fx_key_list_t keys, fx_param_list_t params, fx_action_id_t action_id) {
  /* TABLE ENTRY CHECK KEYS START */
  if (keys.len != 2) {
    SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got invalid keys length %zu while expecting 2, in table control_in_rif.table_bitmap_router\n", keys.len);
    return SX_STATUS_PARAM_ERROR;
  }
  ...
  if (keys.keys[0].key.data != NULL) {
    if (keys.keys[0].key.len != 4) {
      SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got key (0) of length %zu while expecting 4, in table control_in_rif.table_bitmap_router\n", keys.keys[0].key.len);
      return SX_STATUS_PARAM_ERROR;
    }
    if (keys.keys[0].mask.data != NULL) {
      if (keys.keys[0].mask.len != 4) {
        SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got mask (0) of length %zu while expecting 4, in table control_in_rif.table_bitmap_router\n", keys.keys[0].mask.len);
        return SX_STATUS_PARAM_ERROR;
      }
    }
  }
  ...
  if (keys.keys[1].key.data != NULL) {
    if (keys.keys[1].key.len != sizeof(in_addr_t)) {
      SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got key (1) of length %zu while expecting %lu, in table control_in_rif.table_bitmap_router\n", keys.keys[1].key.len, sizeof(in_addr_t));
      return SX_STATUS_PARAM_ERROR;
    }
    if (keys.keys[1].mask.data != NULL) {
      if (keys.keys[1].mask.len != sizeof(in_addr_t)) {
        SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got mask (1) of length %zu while expecting %lu, in table control_in_rif.table_bitmap_router\n", keys.keys[1].mask.len, sizeof(in_addr_t));
        return SX_STATUS_PARAM_ERROR;
      }
    }
  }
...
sx_status_t add_table_entry_control_in_rif_table_bitmap_router(fx_handle_t handle, fx_action_id_t action_id, fx_key_list_t keys,fx_param_list_t params, sx_acl_rule_offset_t* offset_ptr){
  ...
  if(*offset_ptr == table->default_entry_offset){
    SYSLOGF(SX_LOG_ERROR, "Error: add table entry: rule offset same as default entry offset: %u, skipping rule.\n",(unsigned int)*offset_ptr);
    return SX_STATUS_PARAM_ERROR;  }
  /* EXACT OFFSET END */
  sx_status_t           rc = SX_STATUS_SUCCESS;
  sx_flex_acl_flex_rule_t  rule;
...
sx_status_t remove_table_entry_control_in_rif_table_bitmap_router(fx_handle_t handle, sx_acl_rule_offset_t offset){
  const int table_index = 1; // 1 is replaced by macro
  struct acl_table *table = &handle->acl_tables[table_index];
  SYSLOGF(SX_LOG_DEBUG, "remove rule from control_in_rif_table_bitmap_router, offset =  %d\n", offset);
  return remove_table_entry(handle,table, offset);
}

// --------end of control_in_rif_table_bitmap_router----------------

// --------------- control_out_rif_table_l3_vxlan ------------
/************** per table APIs **************/
void fill_custom_bytes_control_out_rif_table_l3_vxlan(struct fx_custom_params *custom) {
  if (custom->is_cb) {
      custom->attr.extraction_points_count = 1;
      custom->attr.extraction_point.extraction_group_type=SX_ACL_CUSTOM_BYTES_EXTRACTION_GROUP_L2;
      custom->attr.extraction_point.params.extraction_l2_group.extraction_l2.extraction_point_type=SX_ACL_CUSTOM_BYTES_EXTRACTION_POINT_TYPE_L2_NONE;
...
//  }
  ...
  /* Key assignment includes custom bytes - for CB SDK should allow P4C to provide a hint */
  /* KEY ASSIGN START */
  table->key_list[0] = FLEX_ACL_KEY_USER_TOKEN;
  /* KEY ASSIGN END */

  sx_status_t rc =  create_p4_table(handle, table, &custom);
  if (rc) {
      SYSLOGF(SX_LOG_ERROR, "Error: create_control_out_rif_table_l3_vxlan, table #%d with acl ID %d\n", table_index, table->acl_id);
     return rc;
  } else {
      SYSLOGF(SX_LOG_DEBUG, "create_control_out_rif_table_l3_vxlan, table #%d with acl ID %d\n", table_index, table->acl_id);
  }
	table->range_table = NULL;  
  pipe_id_list[pipe_ind] = table->acl_id;
  return SX_STATUS_SUCCESS;
...
sx_status_t delete_control_out_rif_table_l3_vxlan(fx_handle_t handle) {
  SYSLOGF(SX_LOG_DEBUG, "delete_control_out_rif_table_l3_vxlan. \n");
  const int table_index = 2; // 2 is replaced by macro
  struct acl_table *table = &handle->acl_tables[table_index];
  struct fx_custom_params custom = {false, 0, 0, 0, 0};
  fill_custom_bytes_control_out_rif_table_l3_vxlan(&custom);
...
sx_status_t fx_control_out_rif_table_l3_vxlan_default_entry_set(fx_handle_t handle, struct acl_table *table, fx_param_list_t params,int action_id){
  sx_acl_rule_offset_t offset  = table->default_entry_offset;
  sx_acl_rule_offset_t *offset_ptr = &offset;
  int sdk_action_count;
  sx_status_t rc = get_sdk_action_num_from_id(action_id,&sdk_action_count);
  ...
  if (rc) {return rc;}
  rule->valid=1;
  SYSLOGF(SX_LOG_DEBUG, "insert rule to control_out_rif_table_l3_vxlan index %d\n", offset);
  /* __TABLE_DEFAULT_ENTRY_KEYS_||_ */
  rule->key_desc_list_p = NULL;
  rule->key_desc_count = 0;
  // init action
...
sx_status_t check_input_lengths_control_out_rif_table_l3_vxlan(fx_key_list_t keys, fx_param_list_t params, fx_action_id_t action_id) {
  /* TABLE ENTRY CHECK KEYS START */
  if (keys.len != 1) {
    SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got invalid keys length %zu while expecting 1, in table control_out_rif.table_l3_vxlan\n", keys.len);
    return SX_STATUS_PARAM_ERROR;
  }

  /* Key #0: FLEX_ACL_KEY_USER_TOKEN */
  if (keys.keys[0].key.data != NULL) {
    if (keys.keys[0].key.len != sizeof(sx_user_token_t)) {
      SYSLOGF(SX_LOG_ERROR, "ERROR: ""Got key (0) of length %zu while expecting %lu, in table control_out_rif.table_l3_vxlan\n", keys.keys[0].key.len, sizeof(sx_user_token_t));
      return SX_STATUS_PARAM_ERROR;
    }
  }
...
void fx_init_key_list_control_out_rif_table_l3_vxlan(fx_key_list_t key_lists[], size_t rule_cnt) {
    for (int i=0; i < rule_cnt; i++) {
  /* INIT RULE START */
      key_lists[i].len = 1;
      key_lists[i].keys = (fx_key_t*) malloc(1 * sizeof(fx_key_t));
...
void fx_deinit_key_list_control_out_rif_table_l3_vxlan(fx_key_list_t key_lists[], size_t rule_cnt) {
    for (int i=0; i < rule_cnt; i++) {
        free(key_lists[i].keys);
    }
}

sx_status_t add_table_entry_control_out_rif_table_l3_vxlan_internal(fx_handle_t handle,struct acl_table *table, sx_flex_acl_flex_rule_t *rule, fx_key_list_t keys,fx_param_list_t params, fx_action_id_t action_id, sx_acl_rule_offset_t* offset_ptr)
{
  int sdk_action_count;
  sx_status_t rc = get_sdk_action_num_from_id(action_id,&sdk_action_count);
  if (rc) {return rc;}
  ...
  /* sx_lib_flex_acl_rule_init allocated 1 * sizeof(sx_flex_acl_key_desc_t) bytes */
  int curr_key=0;
  if (keys.keys[0].key.data != NULL) { /* Key #0: user_token */
     rule->key_desc_list_p[curr_key]=(sx_flex_acl_key_desc_t){
         .key_id =table->key_list[0],
      };
      memcpy(&rule->key_desc_list_p[curr_key].key.user_token, keys.keys[0].key.data, keys.keys[0].key.len);
      /* memset(&rule->key_desc_list_p[curr_key].mask.user_token, 0xFF, keys.keys[0].key.len); // exact match */
      rule->key_desc_list_p[curr_key].mask.user_token = 0xfff; // exact match
      ++curr_key;
  }
  rule->key_desc_count=curr_key; /* key #1 */
  /* TABLE ENTRY KEYS END */
...
/* API for the CLI app - This can be made to be more user friendly */
sx_status_t add_table_entry_control_out_rif_table_l3_vxlan(fx_handle_t handle, fx_action_id_t action_id, fx_key_list_t keys,fx_param_list_t params, sx_acl_rule_offset_t* offset_ptr){
  const int table_index = 2; // 2 is replaced by macro
  struct acl_table *table = &handle->acl_tables[table_index];
  /* EXACT OFFSET START */
  get_exact_offset_internal(table, offset_ptr);
  ...
  sx_status_t           rc = SX_STATUS_SUCCESS;
  sx_flex_acl_flex_rule_t  rule;
  rc = check_input_lengths_control_out_rif_table_l3_vxlan(keys, params, action_id);
  if (rc) return rc;
  /* Convert keys and mask from user friendly format to machine friendly here */
  rc = add_table_entry_control_out_rif_table_l3_vxlan_internal(handle,table,&rule,keys,params,action_id, offset_ptr);
  return rc;
}

sx_status_t remove_table_entry_control_out_rif_table_l3_vxlan(fx_handle_t handle, sx_acl_rule_offset_t offset){
  const int table_index = 2; // 2 is replaced by macro
  struct acl_table *table = &handle->acl_tables[table_index];
  SYSLOGF(SX_LOG_DEBUG, "remove rule from control_out_rif_table_l3_vxlan, offset =  %d\n", offset);
  return remove_table_entry(handle,table, offset);
}

// --------end of control_out_rif_table_l3_vxlan----------------
...
sx_status_t create_pipe_control_in_port(fx_handle_t handle, void *if_list,uint32_t if_list_cnt){
	...
	if(rc) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in creating control_in_port group: %s\n", SX_STATUS_MSG(rc));
	    free(acl_id_list);
	    return rc;
	}
    SYSLOGF(SX_LOG_DEBUG, "Pipe control_in_port was created with %i tables on %i interfaces: ", acl_id_count, if_list_cnt);
	// Binding - port
...
sx_status_t delete_pipe_control_in_port(fx_handle_t handle){
	...
	if (rc1) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in unbinding control_in_port group: %s\n", SX_STATUS_MSG(rc1));
	}
	sx_status_t rc2 = delete_acl_group(handle, SX_ACL_DIRECTION_INGRESS , &handle->control_in_port_group_id);
	if (rc2) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in deleting control_in_port group: %s\n", SX_STATUS_MSG(rc2));
	}
    ...
    SYSLOGF(SX_LOG_DEBUG, "Pipe control_in_port was deleted\n");
...
sx_status_t rebind_pipe_control_in_port(fx_handle_t handle, void *if_list,uint32_t if_list_cnt){
	...
	rc = port_bind(handle,handle->control_in_port_group_id,(boolean_t) true, if_list,if_list_cnt);
	if (rc) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in binding control_in_port group: %s\n", SX_STATUS_MSG(rc));
		return rc;
	}
	control_in_port_bind_list = (port_handle_type *)malloc(sizeof(port_handle_type) * if_list_cnt);
	memcpy(control_in_port_bind_list, if_list, sizeof(port_handle_type) * if_list_cnt);
...
sx_status_t create_pipe_control_in_rif(fx_handle_t handle, void *if_list,uint32_t if_list_cnt){
	...
	if(rc) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in creating control_in_rif group: %s\n", SX_STATUS_MSG(rc));
	    free(acl_id_list);
	    return rc;
	}
    SYSLOGF(SX_LOG_DEBUG, "Pipe control_in_rif was created with %i tables on %i interfaces: ", acl_id_count, if_list_cnt);
	// Binding - port
    if (if_list_cnt > 0) {
...
sx_status_t delete_pipe_control_in_rif(fx_handle_t handle){
	...
	if (rc2) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in deleting control_in_rif group: %s\n", SX_STATUS_MSG(rc2));
	}
	//  compiler replaces below with N table deletes
	sx_status_t rc = SX_STATUS_SUCCESS;
    if (delete_control_in_rif_table_bitmap_classification(handle)) { SYSLOGF(SX_LOG_ERROR, "ERROR in deleting control_in_rif, pipe table num: 0\n"); rc = SX_STATUS_ERROR; }
	if (delete_control_in_rif_table_bitmap_router(handle)) {SYSLOGF(SX_LOG_ERROR,"ERROR in deleting control_in_rif, pipe table num: 1\n"); rc = SX_STATUS_ERROR;}
	for (int i = FX_PIPE_TYPE_MIN; i < FX_PIPE_TYPE_MAX; i++) {
	    handle->pre_pipe_id_list[i] = 0;
	    handle->pre_pipe_id_count[i] = 0;
	}
    SYSLOGF(SX_LOG_DEBUG, "Pipe control_in_rif was deleted\n");
    return rc2 ? rc2 : rc;
}
...
sx_status_t binding_update_pipe_control_in_rif(fx_handle_t handle, void *iface, bool is_add) {
    ...
    if (is_add) {
        if (rif_idx != -1) {
            SYSLOGF(SX_LOG_ERROR, "Failed to add rif %d to fx control_in_rif_bind_list - rif is already in the list\n", rif);
            return SX_STATUS_ERROR;
        }
...
// type rif/port : rif
// auto tables iteration over:
// table name: control_out_rif_table_l3_vxlan ,  index : 0
// if_list type: pptype
...
sx_status_t create_pipe_control_out_rif(fx_handle_t handle, void *if_list,uint32_t if_list_cnt){
	...
	if(rc) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in creating control_out_rif group: %s\n", SX_STATUS_MSG(rc));
	    free(acl_id_list);
	    return rc;
	}
    SYSLOGF(SX_LOG_DEBUG, "Pipe control_out_rif was created with %i tables on %i interfaces: ", acl_id_count, if_list_cnt);
	// Binding - port
    if (if_list_cnt > 0) {
...
sx_status_t delete_pipe_control_out_rif(fx_handle_t handle){
	...
	if (rc2) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in deleting control_out_rif group: %s\n", SX_STATUS_MSG(rc2));
	}
	//  compiler replaces below with N table deletes
	sx_status_t rc = SX_STATUS_SUCCESS;
	if (delete_control_out_rif_table_l3_vxlan(handle)) {SYSLOGF(SX_LOG_ERROR,"ERROR in deleting control_out_rif, pipe table num: 0\n"); rc = SX_STATUS_ERROR;}
	for (int i = FX_PIPE_TYPE_MIN; i < FX_PIPE_TYPE_MAX; i++) {
	    handle->pre_pipe_id_list[i] = 0;
	    handle->pre_pipe_id_count[i] = 0;
	}
    SYSLOGF(SX_LOG_DEBUG, "Pipe control_out_rif was deleted\n");
    return rc2 ? rc2 : rc;
}
...
sx_status_t binding_update_pipe_control_out_rif(fx_handle_t handle, void *iface, bool is_add) {
    ...
    if (is_add) {
        if (rif_idx != -1) {
            SYSLOGF(SX_LOG_ERROR, "Failed to add rif %d to fx control_out_rif_bind_list - rif is already in the list\n", rif);
            return SX_STATUS_ERROR;
        }
...
sx_status_t create_pipe_control_out_port(fx_handle_t handle, void *if_list,uint32_t if_list_cnt){
	...
	if(rc) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in creating control_out_port group: %s\n", SX_STATUS_MSG(rc));
	    free(acl_id_list);
	    return rc;
	}
    SYSLOGF(SX_LOG_DEBUG, "Pipe control_out_port was created with %i tables on %i interfaces: ", acl_id_count, if_list_cnt);
	// Binding - port
...
sx_status_t delete_pipe_control_out_port(fx_handle_t handle){
	...
	if (rc1) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in unbinding control_out_port group: %s\n", SX_STATUS_MSG(rc1));
	}
	sx_status_t rc2 = delete_acl_group(handle, SX_ACL_DIRECTION_EGRESS , &handle->control_out_port_group_id);
	if (rc2) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in deleting control_out_port group: %s\n", SX_STATUS_MSG(rc2));
	}
	//  compiler replaces below with N table deletes
	sx_status_t rc = SX_STATUS_SUCCESS;
	for (int i = FX_PIPE_TYPE_MIN; i < FX_PIPE_TYPE_MAX; i++) {
    ...
    SYSLOGF(SX_LOG_DEBUG, "Pipe control_out_port was deleted\n");
	return rc1 ? rc1 : (rc2 ? rc2 : rc);
}
...
sx_status_t rebind_pipe_control_out_port(fx_handle_t handle, void *if_list,uint32_t if_list_cnt){
	...
	rc = port_bind(handle,handle->control_out_port_group_id,(boolean_t) true, if_list,if_list_cnt);
	if (rc) {
        SYSLOGF(SX_LOG_ERROR, "ERROR in binding control_out_port group: %s\n", SX_STATUS_MSG(rc));
		return rc;
	}
	control_out_port_bind_list = (port_handle_type *)malloc(sizeof(port_handle_type) * if_list_cnt);
	memcpy(control_out_port_bind_list, if_list, sizeof(port_handle_type) * if_list_cnt);
