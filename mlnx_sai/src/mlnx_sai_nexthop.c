...
static sai_status_t mlnx_next_hop_attr_set(_In_ const sai_object_key_t      *key,
                                           ...
                                           void                             *arg);
static sai_status_t mlnx_meta_tunnel_entry_remove(_In_ sx_mac_addr_t *sx_fake_mac,
...
static void next_hop_key_to_str(_In_ sai_object_id_t next_hop_id, _Out_ char *key_str)
{
    ...
    if (SAI_STATUS_SUCCESS != mlnx_object_to_type(next_hop_id, SAI_OBJECT_TYPE_NEXT_HOP, &nexthop_data, (uint8_t*)&ext)) {
...
sai_status_t mlnx_encap_nexthop_get_ecmp(sai_object_id_t  nh,
                                         sai_object_id_t  vrf,
                                         sx_ecmp_id_t    *sx_ecmp)
{
    sai_status_t                   status;
    ...
    *sx_ecmp = 0;
    ...
    for (int32_t i = 0; i < NUMBER_OF_LOCAL_VNETS; ++i) {
        if (db_entry->data.fake_data[i].associated_vrf == vrf) {
            *sx_ecmp = db_entry->data.fake_data[i].sx_fake_nexthop;
            break;
        }
    }
...
static sai_status_t mlnx_encap_nexthop_fake_neighbor_create(_In_ sx_router_interface_t  br_rif,
                                                            ...
                                                            _Out_ sx_neigh_data_t      *sx_neigh_data)
{
    sx_status_t     sx_status;
    ...
    sx_status = sx_api_router_neigh_set(gh_sdk, SX_ACCESS_CMD_ADD, br_rif, fake_ip_addr, sx_neigh_data);
    if (SX_ERR(sx_status)) {
        return sdk_to_sai(sx_status);
    }
...
static sai_status_t mlnx_encap_nexthop_fake_fdb_create(_In_ sx_fid_t                      br_fid,
                                                       _In_ sx_mac_addr_t                *fake_mac,
                                                       _Out_ sx_fdb_uc_mac_addr_params_t *sx_mac_entry)
{
    sx_status_t                 sx_status;
    ...
    memcpy(sx_mac_entry->mac_addr.ether_addr_octet, fake_mac->ether_addr_octet, sizeof(sx_mac_entry->mac_addr.ether_addr_octet));
...
sai_status_t mlnx_encap_nexthop_counter_update(sai_object_id_t nh,
                                               sai_object_id_t vrf,
                                               int32_t         diff)
{
    sai_status_t status;
    ...
    if ((fake_data->counter + diff) < 0) {
        ...
        status = SAI_STATUS_FAILURE;
        goto out;
    } else if ((fake_data->counter + diff) == 0) {
        status = mlnx_encap_nexthop_fake_data_deinit(db_entry->data.tunnel_id,
                                                     db_entry->data.tunnel_vni,
                                                     true,
                                                     fake_data);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed Fake Data deinit.\n");
            goto out;
        }
    } else if (fake_data->counter == 0) {
        status = mlnx_encap_nexthop_fake_data_init(db_entry,
                                                   ...
                                                   fake_data);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed Fake Data init.\n");
            goto out;
        }
    }
                                         ...
                                         _In_ const sai_attribute_t *attr_list)
{
    ...
    if ((SAI_NEXT_HOP_TYPE_TUNNEL_ENCAP == type_attr->s32) && is_tunnel_vxlan) {
        ...
        if (SAI_ERR(sai_status)) {
            SX_LOG_ERR("Failure allocating Encap Nexthop DB entry.\n");
            sai_db_unlock();
            return sai_status;
        }
        ...
        if (SAI_ERR(sai_status)) {
            ...
            SX_LOG_ERR("Failure creating Encap Nexthop OID.\n");
            sai_db_unlock();
            return sai_status;
        }
                                                  ...
                                                  _Out_ uint32_t       *priority)
{
    ...
    memcpy(&action.data.tunnel_encap_params.dst_mac.ether_addr_octet, tunnel_mac.ether_addr_octet, sizeof(tunnel_mac.ether_addr_octet));
                                            ...
                                            _In_ bool store)
{
    ...
    status = mlnx_meta_tunnel_entry_remove(&db_entry->data.sx_fake_mac,
                                           db_entry->data.acl_index);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failure remove meta tunnel entry\n");
        goto out;
    }
    status = mlnx_meta_tunnel_entry_create(&db_entry->data.sx_fake_mac,
                                           db_entry->data.tunnel_id,
                                           ...
                                           &db_entry->data.acl_index);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failure create meta tunnel entry\n");
        goto out;
    }
...
sai_status_t mlnx_encap_nexthop_change_vni(_In_ sai_object_id_t nh,
                                           _In_ uint32_t        vni)
{
    sai_status_t                   status;
    mlnx_encap_nexthop_db_entry_t *db_entry;
    ...
    for (int32_t i = 0; i < NUMBER_OF_LOCAL_VNETS; ++i) {
        if (db_entry->data.fake_data[i].associated_vrf != SAI_NULL_OBJECT_ID) {
            status = mlnx_tunnel_bridge_counter_update(db_entry->data.tunnel_id,
                                                       ...
                                                       db_entry->data.fake_data[i].counter);
            if (SAI_ERR(status)) {
                                           ...
                                           void                             *arg)
{
    ...
    case SAI_NEXT_HOP_ATTR_TUNNEL_MAC:
        ...
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failure changing DMAC.\n");
            goto out;
        }
        break;
