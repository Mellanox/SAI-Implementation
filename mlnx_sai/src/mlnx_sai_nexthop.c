...
static const sai_attribute_entry_t next_hop_attribs[] = {
    { SAI_NEXT_HOP_ATTR_TYPE, true, true, false,
      "Next hop entry type", SAI_ATTR_VAL_TYPE_S32 },
    { SAI_NEXT_HOP_ATTR_IP, true, true, false,
      "Next hop entry IP address", SAI_ATTR_VAL_TYPE_IPADDR },
    { SAI_NEXT_HOP_ATTR_ROUTER_INTERFACE_ID, true, true, false,
      "Next hop entry router interface ID", SAI_ATTR_VAL_TYPE_U32 },
    { END_FUNCTIONALITY_ATTRIBS_ID, false, false, false,
      "", SAI_ATTR_VAL_TYPE_UNDETERMINED }
};

sai_status_t mlnx_next_hop_type_get(_In_ const sai_object_key_t   *key,
                                    _Inout_ sai_attribute_value_t *value,
                                    _In_ uint32_t                  attr_index,
                                    _Inout_ vendor_cache_t        *cache,
                                    void                          *arg);
sai_status_t mlnx_next_hop_ip_get(_In_ const sai_object_key_t   *key,
                                  _Inout_ sai_attribute_value_t *value,
                                  _In_ uint32_t                  attr_index,
                                  _Inout_ vendor_cache_t        *cache,
                                  void                          *arg);
sai_status_t mlnx_next_hop_rif_get(_In_ const sai_object_key_t   *key,
                                   _Inout_ sai_attribute_value_t *value,
                                   _In_ uint32_t                  attr_index,
                                   _Inout_ vendor_cache_t        *cache,
                                   void                          *arg);
...
static const sai_vendor_attribute_entry_t next_hop_vendor_attribs[] = {
    { SAI_NEXT_HOP_ATTR_TYPE,
      ...
      { true, false, false, true },
      mlnx_next_hop_type_get, NULL,
      NULL, NULL },
    { SAI_NEXT_HOP_ATTR_IP,
      { true, false, false, true },
      { true, false, false, true },
      mlnx_next_hop_ip_get, NULL,
      NULL, NULL },
    { SAI_NEXT_HOP_ATTR_ROUTER_INTERFACE_ID,
      { true, false, false, true },
      { true, false, false, true },
      mlnx_next_hop_rif_get, NULL,
      NULL, NULL },
};
static void next_hop_key_to_str(_In_ sai_next_hop_id_t next_hop_id, _Out_ char *key_str)
{
    snprintf(key_str, MAX_KEY_STR_LEN, "next hop id %u", next_hop_id);
}
 ...
 */
sai_status_t mlnx_create_next_hop(_Out_ sai_next_hop_id_t   * next_hop_id,
 ...
 *    Failure status code on error
 */
sai_status_t mlnx_remove_next_hop(_In_ sai_next_hop_id_t next_hop_id)
{
    char key_str[MAX_KEY_STR_LEN];
 ...
 *    Failure status code on error
 */
sai_status_t mlnx_set_next_hop_attribute(_In_ sai_next_hop_id_t next_hop_id, _In_ const sai_attribute_t *attr)
{
    const sai_object_key_t key = { .next_hop_id = next_hop_id };
    char                   key_str[MAX_KEY_STR_LEN];
 ...
 *    Failure status code on error
 */
sai_status_t mlnx_get_next_hop_attribute(_In_ sai_next_hop_id_t   next_hop_id,
                                         _In_ uint32_t            attr_count,
                                  ...
                                  void                          *arg)
{
    ...
    value->ipaddr.addr.ip4 = key->next_hop_id;
