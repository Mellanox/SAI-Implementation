...
static sai_status_t mlnx_sai_buffer_pg0_update_is_required(_In_ const mlnx_port_config_t *port, _Inout_ bool *is_update);
                                                      ...
                                                      _In_ uint32_t                          buff_shared_attr_count)
{
    ...
    if (buff_reserved_attr_count) {
        sai_status = mlnx_sai_buffer_configure_reserved_buffers(port->logical, buff_reserved_attr_arr, buff_reserved_attr_count);
        if (SAI_ERR(sai_status)) {
    ...
    if (buff_shared_attr_count) {
        sai_status = mlnx_sai_buffer_configure_shared_buffers(port->logical, buff_shared_attr_arr, buff_shared_attr_count);
        if (SAI_ERR(sai_status)) {
                                                              ...
                                                              _Inout_ uint32_t *buff_shared_attr_count)
{
    ...
    if (!port || !pgs_buff_reserved_attr_arr || !buff_reserved_attr_count || !pgs_buff_shared_attr_arr || !buff_shared_attr_count) {
        SX_LOG_ERR("NULL param\n");
        sai_status = SAI_STATUS_INVALID_PARAMETER;
        goto out;
    }
    ...
    pg0_buff_reserved_attr.attr.ingress_port_pg_buff_attr.pool_id = g_sai_buffer_db_ptr->buffer_pool_ids.default_ingress_pool_id;
    ...
    pg0_buff_shared_attr.attr.ingress_port_pg_shared_buff_attr.pool_id = g_sai_buffer_db_ptr->buffer_pool_ids.default_ingress_pool_id;
                                                               ...
                                                               _Inout_ uint32_t *buff_attr_count)
{
    ...
    if (!port->admin_state) {
        ...
        pgs_buff_reserved_attr_arr[*buff_attr_count].attr.ingress_port_pg_buff_attr.pipeline_latency.override_default = true;
...
sai_status_t mlnx_sai_buffer_update_db_control_pg9_buff_profile_if_required(_In_ const mlnx_port_config_t *port)
{
    ...
    if (!g_sai_db_ptr->port_pg9_defaults[pg9_profile_idx].is_valid) {
        ...
        g_sai_db_ptr->port_pg9_defaults[pg9_profile_idx].sx_pg_buff_reserved_attr.attr.ingress_port_pg_buff_attr.pg = PG9_PORT_IDX;
