...
#define l2mc_group_db_idx_is_valid(idx) (idx < MLNX_L2MC_GROUP_DB_SIZE)
...
static void l2mcgroup_key_to_str(_In_ sai_object_id_t  object_id,
                                 _Out_ char           *key_str)
{
...
static void l2mcgroup_member_key_to_str(_In_ sai_object_id_t   object_id,
                                        _Out_ char            *key_str)
{
    mlnx_object_id_t *moid = (mlnx_object_id_t*) &object_id;
    ...
    if (SAI_OBJECT_TYPE_L2MC_GROUP_MEMBER != moid->object_type) {
        ...
        snprintf(key_str, MAX_KEY_STR_LEN, "%s (group %x, bport %d)",
                 SAI_TYPE_STR(moid->object_type), moid->id.l2mc_group.db_idx, moid->ext.l2mc_group_member.bport_db_idx);
    }
}
...
static sai_status_t mlnx_l2mc_group_init(_In_ mlnx_l2mc_group_t *l2mc_group)
{
    ...
    sx_status = sx_api_mc_container_set(gh_sdk, SX_ACCESS_CMD_CREATE, &sx_mc_container_id, NULL, 0, &sx_mc_container_attributes);
    if (SX_ERR(sx_status)) {
...
static sai_status_t mlnx_l2mc_group_deinit(_In_ mlnx_l2mc_group_t *l2mc_group)
{
    ...
    if (SX_MC_CONTAINER_ID_CHECK_RANGE(l2mc_group->mc_container)) {
        sx_status = sx_api_mc_container_set(gh_sdk, SX_ACCESS_CMD_DESTROY, &l2mc_group->mc_container, NULL, 0, &sx_mc_container_attributes);
        if (SX_ERR(sx_status)) {
                                               ...
                                               _Inout_ uint32_t              *bports_count)
{
    ...
    sx_status_t                  sx_status;
    sx_mc_container_attributes_t sx_mc_container_attributes;
    sx_mc_next_hop_t             sx_next_hops[MAX_PORTS_DB * 2];
    uint32_t                     next_hops_count = MAX_PORTS_DB * 2, ii;
    ...
    sx_status = sx_api_mc_container_get(gh_sdk, SX_ACCESS_CMD_GET, l2mc_group->mc_container, sx_next_hops, &next_hops_count, &sx_mc_container_attributes);
    if (SX_ERR(sx_status)) {
        SX_LOG_ERR("Faield to get ports from sx_mc_container %x - %s\n", l2mc_group->mc_container, SX_STATUS_MSG(sx_status));
        return sdk_to_sai(sx_status);
    }

    if (*bports_count < next_hops_count) {
        SX_LOG_ERR("bports array size %u < %u\n", *bports_count, next_hops_count);
        return SAI_STATUS_BUFFER_OVERFLOW;
    }

    for (ii = 0; ii < next_hops_count; ii++) {
        status = mlnx_bridge_port_by_log(sx_next_hops[ii].data.log_port, &bports[ii]);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to find bridge port for log port %x in mc container %x\n",
                       sx_next_hops[ii].data.log_port, l2mc_group->mc_container);
                                        ...
                                        _Out_ mlnx_l2mc_group_t **l2mc_group)
{
    ...
    if (!l2mc_group_db_idx_is_valid(db_idx)) {
...
static sai_status_t mlnx_l2mc_group_is_in_use(_In_ mlnx_l2mc_group_t *l2mc_group,
                                              _Out_ bool             *is_in_use)
{
    ...
    sx_status = sx_api_mc_container_get(gh_sdk, SX_ACCESS_CMD_GET, l2mc_group->mc_container, NULL, &next_hops_count, &sx_mc_container_attributes);
    if (SX_ERR(sx_status)) {
                                              ...
                                              _In_ const mlnx_bridge_port_t *bport)
{
    ...
    if (bport->port_type != SAI_BRIDGE_PORT_TYPE_PORT) {
        SX_LOG_ERR("Only SAI_BRIDGE_PORT_TYPE_PORT is supported\n");
        return SAI_STATUS_NOT_SUPPORTED;
    }

    return mlnx_l2mc_group_update(l2mc_group, bport, true);
}
                                              ...
                                              _In_ const mlnx_bridge_port_t *bport)
{
    ...
    return mlnx_l2mc_group_update(l2mc_group, bport, false);
}
                                                  ...
                                                  _Inout_ sai_attribute_t *attr_list)
{
    ...
    l2mcgroup_key_to_str(l2mc_group_id, key_str);
    return sai_get_attributes(&key, key_str, SAI_OBJECT_TYPE_L2MC_GROUP, l2mcgroup_vendor_attribs, attr_count, attr_list);
}
                                                  ...
                                                  _In_ const sai_attribute_t *attr_list)
{
    ...
    find_attrib_in_list(attr_count, attr_list, SAI_L2MC_GROUP_MEMBER_ATTR_L2MC_OUTPUT_ID, &attr_output_id, &attr_index);
    assert(attr_group_id);
