...
static void SAI_dump_tunnel_getdb(_Out_ mlnx_tunnel_map_t  *mlnx_tunnel_map,
...
void SAI_dump_tunnel(_In_ FILE *file)
{
    ...
    if ((!mlnx_tunnel_map) || (!tunnel_db) || (!mlnx_tunneltable)) {
        if (mlnx_tunnel_map) {
    ...
    SAI_dump_tunnel_getdb(mlnx_tunnel_map,
                          tunnel_db,
                          mlnx_tunneltable,
                          &sx_bridge_id);
    dbg_utils_print_module_header(file, "SAI Tunnel");
    SAI_dump_tunnel_map_print(file, mlnx_tunnel_map);
