...
#define MLNX_ACL_FIELD_TCP_UDP_DEFINE(sx_key_id, sx_key_type) \
    MLNX_ACL_FIELD_INFO_DEFINE(sx_key_id, sx_key_type, MLNX_ACL_FIELD_TYPE_TCP_UDP)
...
#define MLNX_ACL_FIELD_ICMPV4_DEFINE(sx_key_id, sx_key_type) \
    MLNX_ACL_FIELD_INFO_DEFINE(sx_key_id, sx_key_type, MLNX_ACL_FIELD_TYPE_ICMPV4)
#define MLNX_ACL_FIELD_ICMPV6_DEFINE(sx_key_id, sx_key_type) \
    MLNX_ACL_FIELD_INFO_DEFINE(sx_key_id, sx_key_type, MLNX_ACL_FIELD_TYPE_ICMPV6)
#define MLNX_ACL_FIELD_TCP_DEFINE(sx_key_id, sx_key_type) \
    MLNX_ACL_FIELD_INFO_DEFINE(sx_key_id, sx_key_type, MLNX_ACL_FIELD_TYPE_TCP)
#define MLNX_ACL_FIELD_INNER_IP_DEFINE(sx_key_id, sx_key_type) \
    MLNX_ACL_FIELD_INFO_DEFINE(sx_key_id, sx_key_type, MLNX_ACL_FIELD_TYPE_INNER_IP)
...
#define MLNX_ACL_FIELD_INNER_L4_DEFINE(sx_key_id, sx_key_type) \
    MLNX_ACL_FIELD_INFO_DEFINE(sx_key_id, sx_key_type, MLNX_ACL_FIELD_TYPE_INNER_L4)
...
typedef enum _mlnx_acl_field_type_t {
    ...
    MLNX_ACL_FIELD_TYPE_IPV4               = (1 << 5) | MLNX_ACL_FIELD_TYPE_IP,
    MLNX_ACL_FIELD_TYPE_NON_IPV4           = (1 << 6),
    MLNX_ACL_FIELD_TYPE_IPV6               = (1 << 7) | MLNX_ACL_FIELD_TYPE_IP,
...
typedef sai_status_t (*mlnx_acl_entry_iter_fn)(_In_ uint32_t  table_idx,
...
typedef sai_status_t (*mlnx_acl_range_create_f)(_In_ sai_acl_range_type_t     type,
...
typedef sai_status_t (*mlnx_acl_range_attr_get_f)(_In_ sx_acl_port_range_id_t  sx_range_id,
...
static sai_status_t mlnx_acl_entry_action_redirect_get(_In_ const sai_object_key_t   *key,
                                                       ...
                                                       _Inout_ vendor_cache_t        *cache,
                                                       void                          *arg);
static sai_status_t mlnx_acl_entry_action_endpoint_get(_In_ const sai_object_key_t   *key,
                                                       _Inout_ sai_attribute_value_t *value,
                                                       _In_ uint32_t                  attr_index,
                                                       _Inout_ vendor_cache_t        *cache,
                                                       void                          *arg);
...
static sx_acl_direction_t mlnx_acl_table_sx_direction_get(_In_ uint32_t table_index);
...
static sai_status_t mlnx_acl_redirect_endpoint_set(_Inout_ sx_flex_acl_flex_rule_t  *sx_rule,
                                                   _In_ uint32_t                     attr_index,
                                                   _In_ const sai_attribute_value_t *data,
                                                   _In_ bool                         on_create);
...
static const mlnx_acl_single_key_field_info_t mlnx_acl_single_key_fields_info[] = {
    ...
    [SAI_ACL_ENTRY_ATTR_FIELD_INNER_ETHER_TYPE] = MLNX_ACL_FIELD_DEFINE(FLEX_ACL_KEY_INNER_ETHERTYPE, inner_ethertype),
    ...
    [SAI_ACL_ENTRY_ATTR_FIELD_INNER_L4_SRC_PORT] =
        MLNX_ACL_FIELD_INNER_L4_DEFINE(FLEX_ACL_KEY_INNER_L4_SOURCE_PORT, inner_l4_source_port),
    [SAI_ACL_ENTRY_ATTR_FIELD_INNER_L4_DST_PORT] =
        MLNX_ACL_FIELD_INNER_L4_DEFINE(FLEX_ACL_KEY_INNER_L4_DESTINATION_PORT, inner_l4_destination_port),
    ...
    [SAI_ACL_ENTRY_ATTR_FIELD_TUNNEL_VNI]     = MLNX_ACL_FIELD_DEFINE(FLEX_ACL_KEY_VNI_KEY, vni_key),
#else /* ifndef _WIN32 */
    { 0, 0, 0 }
#endif /* ifndef _WIN32 */
};
...
static const sai_vendor_attribute_entry_t acl_entry_vendor_attribs[] = {
    ...
    { SAI_ACL_ENTRY_ATTR_ACTION_ENDPOINT_IP,
      { true, false, true, true },
      { true, false, true, true },
      mlnx_acl_entry_action_endpoint_get, NULL,
...
static sai_status_t mlnx_acl_field_stage_is_supported(_In_ sai_acl_stage_t        table_stage,
                                                            ...
                                                            _Out_ bool               *is_supported)
{
    ...
    if (mlnx_acl_field_is_not_trivial(attr_id)) {
        ...
        status = mlnx_acl_field_stage_is_supported(table_stage, field_stage, is_supported);
        if (SAI_ERR(status)) {
            return status;
        }
                                                ...
                                                _Inout_ uint32_t           *sx_key_count)
{
    ...
    for (ii = 0; ii < attr_count; ii++) {
        ...
        status = mlnx_acl_field_info_data_fetch(attr_list[ii].id, &field_type, sx_keys,
                                                &new_key_count, &field_supported_stage);
        if (SAI_ERR(status)) {
            return status;
        }

        status = mlnx_acl_field_stage_is_supported(table_stage, field_supported_stage, &is_field_stage_supported);
        if (SAI_ERR(status)) {
            return status;
        }
    ...
    *fields_types = MLNX_ACL_FIELD_TYPE_EMPTY;
    ...
    for (ii = 0; ii < sx_descs_count; ii++) {
        ...
        case FLEX_ACL_KEY_IS_IP_V4:
            (*fields_types) |= sx_descs[ii].key.is_ip_v4 ? MLNX_ACL_FIELD_TYPE_IPV4 : MLNX_ACL_FIELD_TYPE_NON_IPV4;
            break;
                                                       ...
                                                       void                          *arg)
{
    ...
    pbs_info = acl_db_entry(acl_entry_index).pbs_info;
    ...
    for (ii = 0; ii < rule.action_count; ii++) {
        if ((rule.action_list_p[ii].type == SX_FLEX_ACL_ACTION_PBS) ||
            ...
            (rule.action_list_p[ii].type == SX_FLEX_ACL_ACTION_NVE_TUNNEL_ENCAP)) {
            if (is_action_present) {
    ...
    if ((rule.action_list_p[action_index].type == SX_FLEX_ACL_ACTION_PBS) && (!MLNX_ACL_PBS_INFO_IS_VALID(pbs_info))) {
        SX_LOG_ERR("Invalid ACL entry %lx DB state: SX_FLEX_ACL_ACTION_PBS is present but pbs_info is not valid\n",
                   key->key.object_id);
        status = SAI_STATUS_FAILURE;
        goto out;
    }
    ...
    case SAI_ACL_ENTRY_ATTR_ACTION_REDIRECT:
        status = mlnx_acl_redirect_sx_to_sai(&rule.action_list_p[action_index], pbs_info, &value->aclaction);
        break;
                                                       ...
                                                       void                          *arg)
{
    ...
    mlnx_acl_flex_rule_action_find(&rule, SX_FLEX_ACL_ACTION_NVE_TUNNEL_ENCAP, &action_index, &is_action_present);

    if (is_action_present) {
        value->aclaction.enable = true;
        status                  = mlnx_translate_sdk_ip_address_to_sai(
            &rule.action_list_p[action_index].fields.action_nve_tunnel_encap.underlay_dip,
            &value->aclaction.parameter.ipaddr);
        if (SAI_ERR(status)) {
            goto out;
        }
    } else {
                                                     ...
                                                     void                          *arg)
{
    ...
    sx_span_session_id_t           sx_db_span_session = MLNX_ACL_SX_SPAN_SESSION_INVALID;
    ...
    sx_db_span_session = acl_db_entry(acl_entry_index).sx_span_session;
    ...
    status = mlnx_acl_mirror_config_validate(&flex_acl_rule, sx_action_type, sx_db_span_session);
    if (SAI_ERR(status)) {
        goto out;
    }

    if (MLNX_ACL_SX_SPAN_SESSION_IS_VALID(sx_db_span_session)) {
        value->aclaction.enable = true;
        status = mlnx_create_object(SAI_OBJECT_TYPE_MIRROR_SESSION, sx_db_span_session, NULL,
                                             ...
                                             void                             *arg)
{
    ...
    status = mlnx_acl_field_stage_is_supported(acl_db_table(
...
static sai_status_t mlnx_acl_entry_action_policer_check_allowed(_In_ uint32_t acl_entry_idx)
{
    sx_span_session_id_t   span_session;
    mlnx_mirror_policer_t *mirror_policer;


    assert(acl_entry_index_check_range(acl_entry_idx));

    span_session = acl_db_entry(acl_entry_idx).sx_span_session;

    if (!MLNX_ACL_SX_SPAN_SESSION_IS_VALID(span_session)) {
        return SAI_STATUS_SUCCESS;
    }

    mirror_policer = &g_sai_db_ptr->mirror_policer[span_session];
                                              ...
                                              void                             *arg)
{
    ...
    switch ((int64_t)arg) {
    case SAI_ACL_ENTRY_ATTR_ACTION_SET_POLICER:
        status = mlnx_acl_entry_action_policer_check_allowed(acl_entry_index);
        if (SAI_ERR(status)) {
            goto out;
        }
                                                       ...
                                                       void                             *arg)
{
    ...
    status = mlnx_acl_redirect_endpoint_set(&flex_acl_rule, 0, value, false);
    if (SAI_ERR(status)) {
        goto out;
    }
                                               ...
                                               _In_ sx_span_session_id_t       sx_span_session)
{
    ...
    sx_status = sx_api_acl_set(gh_sdk, SX_ACCESS_CMD_CREATE, SX_ACL_TYPE_PACKET_TYPES_AGNOSTIC, sx_direction, &region_group, &acl_id);
    if (SX_ERR(sx_status)) {
                                               ...
                                               _In_ sx_acl_direction_t         sx_direction)
{
    ...
    sx_status = sx_api_acl_set(gh_sdk, SX_ACCESS_CMD_DESTROY, SX_ACL_TYPE_PACKET_TYPES_AGNOSTIC, sx_direction, NULL, &acl->acl);
    if (SX_ERR(sx_status)) {
...
static sai_status_t mlnx_acl_mirror_policer_extra_actions_add(_In_ sx_flex_acl_flex_rule_t *sx_rule,
                                                              ...
                                                              _In_ sx_span_session_id_t     sx_span_session)
{
    ...
    mlnx_acl_flex_rule_action_find(sx_rule, SX_FLEX_ACL_ACTION_POLICER, &policer_action_idx, &is_policer_action_present);
    if (is_policer_action_present) {
        SX_LOG_ERR("Failed to add extra action for mirror session %d policer %lx - ACTION_POLICER already exists\n",
                   sx_span_session, mirror_policer->policer_oid);
        return SAI_STATUS_FAILURE;
    }
    ...
    if (is_goto_action_present) {
        SX_LOG_ERR("Failed to add extra action for mirror session %d policer %lx - ACTION_GOTO already exists\n",
                   sx_span_session, mirror_policer->policer_oid);
        return SAI_STATUS_FAILURE;
    }
    ...
    sx_rule->action_list_p[*action_count].type = SX_FLEX_ACL_ACTION_POLICER;
    sx_rule->action_list_p[*action_count].fields.action_policer.policer_id = sx_policer_id;
    (*action_count)++;
    ...
    sx_rule->action_list_p[*action_count].type                               = SX_FLEX_ACL_ACTION_GOTO;
    sx_rule->action_list_p[*action_count].fields.action_goto.acl_group_id    = extra_acl->acl_group;
    sx_rule->action_list_p[*action_count].fields.action_goto.goto_action_cmd = SX_ACL_ACTION_GOTO_CALL;
    (*action_count)++;
...
static sai_status_t mlnx_acl_entry_mirror_action_bind(_In_ sx_flex_acl_flex_rule_t *sx_rule,
                                                      _Inout_ uint32_t             *action_count,
                                                      _In_ sx_acl_direction_t       sx_direction,
                                                      _In_ sx_span_session_id_t     sx_span_session)
{
    ...
    if (mirror_policer->policer_oid == SAI_NULL_OBJECT_ID) {
        sx_rule->action_list_p[*action_count].type = sx_mirror_action;

        if (sx_mirror_action == SX_FLEX_ACL_ACTION_MIRROR) {
            sx_rule->action_list_p[*action_count].fields.action_mirror.session_id = sx_span_session;
        } else { /* SX_FLEX_ACL_ACTION_EGRESS_MIRROR */
            sx_rule->action_list_p[*action_count].fields.action_egress_mirror.session_id = sx_span_session;
        }

        (*action_count)++;
    } else {
        status = mlnx_acl_mirror_policer_extra_actions_add(sx_rule, action_count, sx_direction, sx_span_session);
        if (SAI_ERR(status)) {
            return status;
        }
    }
                                                     ...
                                                     void                             *arg)
{
    ...
    if ((acl_db_table(acl_table_index).stage != acl_stage) &&
            (mlnx_acl_table_sx_direction_get(acl_table_index) != SX_ACL_DIRECTION_RIF_EGRESS)) {
    ...
    sx_direction       = mlnx_acl_table_sx_direction_get(acl_table_index);
    ...
    if (MLNX_ACL_SX_SPAN_SESSION_IS_VALID(new_session_id)) {
        status = mlnx_acl_entry_mirror_action_bind(&flex_acl_rule, &flex_acl_rule.action_count, sx_direction, new_session_id);
        if (SAI_ERR(status)) {
                                                                ...
                                                                _In_ void     *opaque)
{
    ...
    sx_direction = mlnx_acl_table_sx_direction_get(table_idx);
    ...
    status = mlnx_acl_entry_mirror_action_bind(&sx_rule, &sx_rule.action_count, sx_direction, sx_span_session);
    if (SAI_ERR(status)) {
        goto out;
    }
                                   ...
                                   _In_ const sai_attribute_t *attr_list)
{
    ...
    sx_span_session_id_t         sx_span_session              = MLNX_ACL_SX_SPAN_SESSION_INVALID;
    sx_acl_direction_t           sx_direction;
    ...
    const sai_attribute_value_t *tunnel_endpoint = NULL;
    ...
    uint32_t                     packet_action_index, action_counter_index, action_redirect_index,
                                 action_redirect_list_index, attr_index;
    ...
    uint32_t key_desc_index = 0, flex_action_index = 0;
    uint16_t trap_id        = SX_TRAP_ID_ACL_MIN;
    ...
    if (SAI_STATUS_SUCCESS ==
        ...
        if (action_redirect->aclaction.enable == true) {
            status = mlnx_sai_acl_redirect_action_create(action_redirect->aclaction.parameter.oid,
                                                         action_redirect_index, &pbs_info,
    ...
    find_attrib_in_list(attr_count, attr_list, SAI_ACL_ENTRY_ATTR_ACTION_ENDPOINT_IP, &tunnel_endpoint, &attr_index);
    status = mlnx_acl_redirect_endpoint_set(&flex_acl_rule, attr_index, tunnel_endpoint, true);
    if (SAI_ERR(status)) {
        goto out;
    }
    ...
    if (SAI_STATUS_SUCCESS ==
        ...
        if (action_redirect_list->aclaction.enable == true) {
            ...
            status = mlnx_acl_pbs_ports_create_or_get(action_redirect_list->aclaction.parameter.objlist.list,
    ...
    if (SAI_STATUS_SUCCESS ==
        ...
        if (action_flood->aclaction.enable == true) {
            ...
            status = mlnx_acl_flood_pbs_create_or_get(&pbs_id, &pbs_info);
            if (SAI_STATUS_SUCCESS != status) {
                goto out;
            }
    ...
    if (SAI_STATUS_SUCCESS ==
        ...
        if (action_mirror_ingress->aclaction.enable == true) {
            ...
            status = mlnx_acl_entry_mirror_action_bind(&flex_acl_rule, &flex_action_index,
                                                       sx_direction, ingress_session_id);
            if (SAI_ERR(status)) {
                goto out;
            }

            sx_span_session = ingress_session_id;
        }
    }
    ...
    if (SAI_STATUS_SUCCESS ==
        ...
        if (action_mirror_egress->aclaction.enable == true) {
            ...
            status = mlnx_acl_entry_mirror_action_bind(&flex_acl_rule, &flex_action_index,
                                                       sx_direction, egress_session_id);
            if (SAI_ERR(status)) {
                goto out;
            }

            sx_span_session = egress_session_id;
        }
    }
    ...
    acl_db_entry(acl_entry_index).pbs_info            = pbs_info;
    acl_db_entry(acl_entry_index).sx_span_session     = sx_span_session;
    ...
    status = mlnx_acl_mirror_session_ref_inc(sx_span_session, sx_direction);
    if (SAI_ERR(status)) {
        goto out;
    }
    ...
    status = mlnx_acl_pbs_info_port_refs_get(pbs_info, ports_refs);
    if (SAI_ERR(status)) {
        goto out;
    }
...
out:
    if (SAI_STATUS_SUCCESS != status) {
        ...
        mlnx_acl_pbs_info_delete(pbs_info);
    }
...
static sai_status_t mlnx_delete_acl_entry_data(_In_ uint32_t table_index, _In_ uint32_t entry_index)
{
    ...
    sx_direction = mlnx_acl_table_sx_direction_get(table_index);
...
static sx_acl_direction_t mlnx_acl_table_sx_direction_get(_In_ uint32_t table_index)
{
    ...
    list = &acl_db_table(table_index).bind_point_types;
    ...
    if (acl_db_table(table_index).stage == SAI_ACL_STAGE_INGRESS) {
        if (is_rif) {
                                                   ...
                                                   _In_ bool                         on_create)
{
    ...
    bool          is_action_present, enable;

    assert(sx_rule);

    enable = data && data->aclaction.enable;

    mlnx_acl_flex_rule_action_find(sx_rule, SX_FLEX_ACL_ACTION_NVE_TUNNEL_ENCAP, &action_index, &is_action_present);

    if (is_action_present) {
        if (enable) {
            sx_ip  = &sx_rule->action_list_p[action_index].fields.action_nve_tunnel_encap.underlay_dip;
            status = mlnx_translate_sai_ip_address_to_sdk(&data->aclaction.parameter.ipaddr, sx_ip);
            if (SAI_ERR(status)) {
                return SAI_STATUS_INVALID_ATTR_VALUE_0 + attr_index;
            }
        } else {
