...
#define MLNX_ACL_ACTION_CTX_EMPTY                                     \
    {                                                                 \
        ...
        .sx_mc_container_port_isolation = SX_MC_CONTAINER_ID_INVALID, \
...
typedef struct _mlnx_acl_action_ctx_t {
    sx_mc_container_id_t sx_mc_container_port_isolation;
...
typedef sai_status_t (*mlnx_acl_ip_ident_key_descr_add_f)(_In_ const sai_attribute_value_t *ip_ident,
                                                          _Inout_ sx_flex_acl_key_desc_t   *sx_key_descs,
                                                          _Inout_ uint32_t                 *key_desc_index);
typedef sai_status_t (*mlnx_acl_ip_ident_key_descr_create_f)(_In_ uint16_t                   value,
                                                             _In_ uint16_t                   mask,
                                                             _Inout_ sx_flex_acl_key_desc_t *key_descs,
                                                             _Inout_ uint32_t                key_desc_count);
typedef sai_status_t (*mlnx_acl_entry_ip_ident_get_f)(_In_ sx_flex_acl_flex_rule_t  *flex_acl_rule,
                                                      _Inout_ sai_attribute_value_t *value);
typedef sai_status_t (*mlnx_acl_entry_ip_ident_set_f)(_In_ const sai_attribute_value_t *value,
                                                      _Inout_ sx_flex_acl_flex_rule_t  *flex_acl_rule);
typedef sai_status_t (*mlnx_acl_ip_ident_key_create_or_get_f)(_Inout_ sx_acl_key_t *keys,
                                                              _Inout_ uint32_t     *key_index);
typedef sai_status_t (*mlnx_acl_ip_ident_key_ref_remove_f)(void);
typedef sai_status_t (*mlnx_acl_entry_udf_key_descr_to_data_f)(_Inout_ sai_attribute_value_t      *value,
                                                               _In_ const sx_flex_acl_flex_rule_t *flex_acl_rule,
                                                               _In_ const sx_acl_key_t            *custom_byte_keys,
                                                               _In_ uint32_t                       custom_byte_count);
typedef sai_status_t (*mlnx_acl_entry_udf_data_to_key_descr_f)(_In_ const sai_attribute_value_t *value,
                                                               _Inout_ sx_flex_acl_flex_rule_t  *flex_acl_rule,
                                                               _In_ const sx_acl_key_t          *custom_byte_keys,
                                                               _In_ uint32_t                     custom_byte_count);
typedef  sai_status_t (*mlnx_acl_entry_udf_data_to_key_descr_list_f)(_In_ const sai_acl_field_data_t *value,
                                                                     _In_ const sx_acl_key_t         *custom_byte_keys,
                                                                     _In_ uint32_t                    custom_byte_count,
...
typedef struct _mlnx_acl_cb_list_t {
    ...
    mlnx_acl_ip_ident_key_descr_add_f           ip_ident_key_descr_add;
    mlnx_acl_ip_ident_key_descr_create_f        ip_ident_key_descr_create;
    mlnx_acl_entry_ip_ident_get_f               ip_ident_get;
    mlnx_acl_entry_ip_ident_set_f               ip_ident_set;
    mlnx_acl_ip_ident_key_create_or_get_f       ip_ident_key_create_or_get;
    mlnx_acl_ip_ident_key_ref_remove_f          ip_ident_key_ref_remove;
    mlnx_acl_entry_udf_key_descr_to_data_f      udf_key_descr_to_data;
    mlnx_acl_entry_udf_data_to_key_descr_f      udf_data_to_key_descr;
    mlnx_acl_entry_udf_data_to_key_descr_list_f udf_data_to_key_descr_list;
} mlnx_acl_cb_list_t;
...
static sai_status_t mlnx_acl_entry_action_l4port_set(_In_ const sai_object_key_t      *key,
                                                     _In_ const sai_attribute_value_t *value,
                                                     void                             *arg);
static sai_status_t mlnx_acl_entry_action_hash_set(_In_ const sai_object_key_t      *key,
                                                   _In_ const sai_attribute_value_t *value,
                                                   void                             *arg);
...
static sai_status_t mlnx_acl_field_port_list_to_sx(_In_ const sai_object_list_t *port_list,
                                                   _Inout_ sx_port_log_id_t     *sx_port_list,
...
static sai_status_t mlnx_acl_sx_mc_container_create_by_port_oid(_In_ const sai_object_list_t *port_obj_list,
                                                                _Out_ sx_mc_container_id_t   *sx_mc_container_id);
static sai_status_t mlnx_acl_sx_mc_container_create_by_log_port(_In_ const sx_port_log_id_t *log_port_list,
                                                                _In_ const uint32_t          log_port_count,
                                                                _Out_ sx_mc_container_id_t  *sx_mc_container_id);
static sai_status_t mlnx_acl_sx_mc_container_update_by_port_oid(_In_ const sai_object_list_t *port_obj_list,
                                                                _In_ sx_mc_container_id_t     sx_mc_container_id);
static sai_status_t mlnx_acl_sx_mc_container_update_by_log_port(_In_ const sx_port_log_id_t *log_port_list,
                                                                _In_ const uint32_t          log_port_count,
                                                                _In_ sx_mc_container_id_t    sx_mc_container_id);
...
static sai_status_t mlnx_acl_entry_udf_attrs_validate_and_fetch(_In_ const sai_attribute_value_t *udf_attr,
                                                                ...
                                                                _In_ uint32_t                     acl_table_index,
                                                                _Out_ sx_acl_key_t               *flex_acl_key_ids,
                                                                _Out_ uint32_t                   *flex_acl_key_ids_num,
...
static sai_status_t mlnx_acl_range_attr_get_spc2(_In_ sx_acl_port_range_id_t  sx_range_id,
                                                 _In_ sai_attr_id_t           attr_id,
                                                 _Out_ sai_attribute_value_t *value);
static sai_status_t mlnx_acl_ip_ident_key_descr_add_spc(_In_ const sai_attribute_value_t *ip_ident,
                                                        _Inout_ sx_flex_acl_key_desc_t   *sx_key_descs,
                                                        _Inout_ uint32_t                 *key_desc_index);
static sai_status_t mlnx_acl_ip_ident_key_descr_add_spc2(_In_ const sai_attribute_value_t *ip_ident,
                                                         _Inout_ sx_flex_acl_key_desc_t   *sx_key_descs,
                                                         _Inout_ uint32_t                 *key_desc_index);
static sai_status_t mlnx_acl_ip_ident_key_desc_create_spc(_In_ uint16_t                   value,
                                                          _In_ uint16_t                   mask,
                                                          _Inout_ sx_flex_acl_key_desc_t *key_descs,
                                                          _Inout_ uint32_t                key_desc_count);
static sai_status_t mlnx_acl_ip_ident_key_desc_create_spc2(_In_ uint16_t                   value,
                                                           _In_ uint16_t                   mask,
                                                           _Inout_ sx_flex_acl_key_desc_t *key_descs,
                                                           _Inout_ uint32_t                key_desc_count);

static sai_status_t mlnx_acl_ip_ident_get_spc(_In_ sx_flex_acl_flex_rule_t  *flex_acl_rule,
                                              _Inout_ sai_attribute_value_t *value);
static sai_status_t mlnx_acl_ip_ident_get_spc2(_In_ sx_flex_acl_flex_rule_t  *flex_acl_rule,
                                               _Inout_ sai_attribute_value_t *value);
static sai_status_t mlnx_acl_ip_ident_set_spc(_In_ const sai_attribute_value_t *value,
                                              _Inout_ sx_flex_acl_flex_rule_t  *flex_acl_rule);
static sai_status_t mlnx_acl_ip_ident_set_spc2(_In_ const sai_attribute_value_t *value,
                                               _Inout_ sx_flex_acl_flex_rule_t  *flex_acl_rule);
static sai_status_t mlnx_acl_ip_ident_key_create_or_get_spc(_Inout_ sx_acl_key_t *keys,
                                                            _Inout_ uint32_t     *key_index);
static sai_status_t mlnx_acl_ip_ident_key_create_or_get_spc2(_Inout_ sx_acl_key_t *keys,
                                                             _Inout_ uint32_t     *key_index);
static sai_status_t mlnx_acl_ip_ident_key_ref_remove_spc(void);
static sai_status_t mlnx_acl_ip_ident_key_ref_remove_spc2(void);
static sai_status_t mlnx_acl_udf_key_descr_to_data_spc(_Inout_ sai_attribute_value_t      *value,
                                                       _In_ const sx_flex_acl_flex_rule_t *flex_acl_rule,
                                                       _In_ const sx_acl_key_t            *custom_byte_keys,
                                                       _In_ uint32_t                       custom_byte_count);
static sai_status_t mlnx_acl_udf_key_descr_to_data_spc2(_Inout_ sai_attribute_value_t      *value,
                                                        _In_ const sx_flex_acl_flex_rule_t *flex_acl_rule,
                                                        _In_ const sx_acl_key_t            *custom_byte_keys,
                                                        _In_ uint32_t                       custom_byte_count);
static sai_status_t mlnx_acl_udf_data_to_key_descr_spc(_In_ const sai_attribute_value_t *value,
                                                       _Inout_ sx_flex_acl_flex_rule_t  *flex_acl_rule,
                                                       _In_ const sx_acl_key_t          *custom_byte_keys,
                                                       _In_ uint32_t                     custom_byte_count);
static sai_status_t mlnx_acl_udf_data_to_key_descr_spc2(_In_ const sai_attribute_value_t *value,
                                                        _Inout_ sx_flex_acl_flex_rule_t  *flex_acl_rule,
                                                        _In_ const sx_acl_key_t          *custom_byte_keys,
                                                        _In_ uint32_t                     custom_byte_count);

static sai_status_t mlnx_acl_entry_udf_data_to_key_descr_list_spc(_In_ const sai_acl_field_data_t *value,
                                                                  _In_ const sx_acl_key_t         *flex_acl_key_ids,
                                                                  _In_ uint32_t                    custom_byte_count,
                                                                  _Out_ sx_flex_acl_key_desc_t    *key_desc,
                                                                  _Inout_ uint32_t                *key_desc_count);
static sai_status_t mlnx_acl_entry_udf_data_to_key_descr_list_spc2(_In_ const sai_acl_field_data_t *value,
                                                                   _In_ const sx_acl_key_t         *flex_acl_key_ids,
                                                                   _In_ uint32_t                    custom_byte_count,
                                                                   _Out_ sx_flex_acl_key_desc_t    *key_desc,
                                                                   _Inout_ uint32_t                *key_desc_count);
...
static const mlnx_acl_cb_list_t               mlnx_acl_cb_sp = {
    ...
    mlnx_acl_ip_ident_key_descr_add_spc,
    mlnx_acl_ip_ident_key_desc_create_spc,
    mlnx_acl_ip_ident_get_spc,
    mlnx_acl_ip_ident_set_spc,
    mlnx_acl_ip_ident_key_create_or_get_spc,
    mlnx_acl_ip_ident_key_ref_remove_spc,
    mlnx_acl_udf_key_descr_to_data_spc,
    mlnx_acl_udf_data_to_key_descr_spc,
    mlnx_acl_entry_udf_data_to_key_descr_list_spc,
};
static const mlnx_acl_cb_list_t               mlnx_acl_cb_sp2 = {
    ...
    mlnx_acl_ip_ident_key_descr_add_spc2,
    mlnx_acl_ip_ident_key_desc_create_spc2,
    mlnx_acl_ip_ident_get_spc2,
    mlnx_acl_ip_ident_set_spc2,
    mlnx_acl_ip_ident_key_create_or_get_spc2,
    mlnx_acl_ip_ident_key_ref_remove_spc2,
    mlnx_acl_udf_key_descr_to_data_spc2,
    mlnx_acl_udf_data_to_key_descr_spc2,
    mlnx_acl_entry_udf_data_to_key_descr_list_spc2,
};
...
static sai_status_t mlnx_acl_action_ip_to_sai(_In_ sai_acl_entry_attr_t           action,
                                              ...
                                              _In_ uint32_t                       attr_index,
                                              _Out_ sai_acl_action_data_t        *action_data);
static sai_status_t mlnx_acl_action_hash_to_sx(_In_ sai_acl_entry_attr_t         action,
                                               _In_ const sai_acl_action_data_t *action_data,
                                               _In_ uint32_t                     attr_index,
                                               _In_ const acl_table_db_t        *table,
                                               _Inout_ mlnx_acl_action_ctx_t    *ctx,
                                               _Out_ sx_flex_acl_flex_action_t  *sx_action_list,
                                               _Inout_ uint32_t                 *sx_action_count);
static sai_status_t mlnx_acl_action_hash_to_sai(_In_ sai_acl_entry_attr_t           action,
                                                _In_ const sx_flex_acl_flex_rule_t *sx_rule,
                                                _In_ const acl_entry_db_t          *entry,
                                                _In_ const acl_table_db_t          *table,
                                                _In_ uint32_t                       attr_index,
...
static sai_status_t mlnx_acl_entry_action_fn_get(_In_ const sai_object_key_t   *key,
                                                 ...
                                                 _Inout_ vendor_cache_t        *cache,
                                                 void                          *arg);
static sai_status_t mlnx_acl_entry_action_isolation_group_get(_In_ const sai_object_key_t   *key,
                                                              _Inout_ sai_attribute_value_t *value,
                                                              _In_ uint32_t                  attr_index,
                                                              _Inout_ vendor_cache_t        *cache,
                                                              void                          *arg);
static sai_status_t mlnx_acl_entry_action_isolation_group_set(_In_ const sai_object_key_t      *key,
                                                              _In_ const sai_attribute_value_t *value,
                                                              void                             *arg);
static sai_status_t mlnx_acl_entry_action_trap_get(_In_ const sai_object_key_t   *key,
                                                   _Inout_ sai_attribute_value_t *value,
                                                   _In_ uint32_t                  attr_index,
                                                   _Inout_ vendor_cache_t        *cache,
                                                   void                          *arg);
static sai_status_t mlnx_acl_entry_action_trap_set(_In_ const sai_object_key_t      *key,
                                                   _In_ const sai_attribute_value_t *value,
                                                   void                             *arg);
extern sai_status_t mlnx_fine_grained_hash_create(_In_ sai_object_id_t                hash_id,
                                                  _In_ sx_flex_acl_action_hash_type_t action_type,
                                                  _Inout_ sx_flex_acl_flex_action_t  *action_list,
                                                  _Inout_ uint32_t                   *action_num);
extern sai_status_t gp_registers_delete(_Inout_ mlnx_sai_fg_hash_field_t *fields_list);

static sai_status_t mlnx_convert_acl_action_to_sai(bool                         is_forward_action_present,
                                                   sx_flex_acl_forward_action_t forward_action,
                                                   bool                         is_trap_action_present,
...
static mlnx_acl_action_info_t mlnx_acl_action_info[] = {
    ...
    [SAI_ACL_ENTRY_ATTR_ACTION_SET_USER_TRAP_ID] =
        MLNX_ACL_ACTION_INFO_DEFINE_WITH_FNS(mlnx_acl_action_pkt_act_to_sx, NULL),
...
{ acl_table_vendor_attribs, OBJ_ATTRS_ENUMS_INFO(acl_table_enum_info), OBJ_STAT_CAP_INFO_EMPTY()};
...
static const sai_vendor_attribute_entry_t acl_entry_vendor_attribs[] = {
    ...
    { SAI_ACL_ENTRY_ATTR_ACTION_SET_ISOLATION_GROUP,
      { true, false, true, true},
      { true, false, true, true},
      mlnx_acl_entry_action_isolation_group_get, NULL,
      mlnx_acl_entry_action_isolation_group_set, NULL },
    { SAI_ACL_ENTRY_ATTR_ACTION_SET_USER_TRAP_ID,
      { true, false, true, true},
      { true, false, true, true},
      mlnx_acl_entry_action_trap_get, NULL,
      mlnx_acl_entry_action_trap_set, NULL },
    { SAI_ACL_ENTRY_ATTR_ACTION_SET_VRF,
      { true, false, true, true},
      { true, false, true, true},
    ...
    { SAI_ACL_ENTRY_ATTR_ACTION_SET_LAG_HASH_ID,
      { true, false, true, true},
      { true, false, true, true},
      mlnx_acl_entry_action_fn_get, (void*)SAI_ACL_ENTRY_ATTR_ACTION_SET_LAG_HASH_ID,
      mlnx_acl_entry_action_hash_set,  (void*)SAI_ACL_ENTRY_ATTR_ACTION_SET_LAG_HASH_ID},
    { SAI_ACL_ENTRY_ATTR_ACTION_SET_ECMP_HASH_ID,
      { true, false, true, true},
      { true, false, true, true},
      mlnx_acl_entry_action_fn_get, (void*)SAI_ACL_ENTRY_ATTR_ACTION_SET_ECMP_HASH_ID,
      mlnx_acl_entry_action_hash_set,  (void*)SAI_ACL_ENTRY_ATTR_ACTION_SET_ECMP_HASH_ID},
    { END_FUNCTIONALITY_ATTRIBS_ID,
      { false, false, false, false },
      { false, false, false, false },
      NULL, NULL,
...
{ acl_entry_vendor_attribs, OBJ_ATTRS_ENUMS_INFO(acl_entry_enum_info), OBJ_STAT_CAP_INFO_EMPTY()};
...
{ acl_range_vendor_attribs, OBJ_ATTRS_ENUMS_INFO(acl_range_enum_info), OBJ_STAT_CAP_INFO_EMPTY()};
...
{ acl_counter_vendor_attribs, OBJ_ATTRS_ENUMS_INFO_EMPTY(), OBJ_STAT_CAP_INFO_EMPTY()};
...
{ acl_group_vendor_attribs, OBJ_ATTRS_ENUMS_INFO(acl_table_group_enum_info), OBJ_STAT_CAP_INFO_EMPTY()};
...
{ acl_group_member_vendor_attribs, OBJ_ATTRS_ENUMS_INFO_EMPTY(), OBJ_STAT_CAP_INFO_EMPTY()};
...
sai_status_t mlnx_acl_disconnect(void)
{
    ...
    SX_LOG_DBG("Deinitializing ACL on switch disconnect\n");
                                                      ...
                                                      _Out_ sx_mc_container_id_t  *sx_mc_container_id)
{
    ...
    for (ii = 0; ii < log_port_count; ii++) {
        ...
        sx_mc_next_hops[ii].data.log_port = log_port_list[ii];
    }
                                                                ...
                                                                _Out_ sx_mc_container_id_t  *sx_mc_container_id)
{
    ...
    assert(log_port_list);
    assert(sx_mc_container_id);

    status = mlnx_acl_sx_mc_container_set_impl(SX_ACCESS_CMD_CREATE, log_port_list, log_port_count,
                                                                ...
                                                                _Out_ sx_mc_container_id_t   *sx_mc_container_id)
{
    ...
    uint32_t          log_port_count = MAX_PORTS;

    assert(NULL != port_obj_list);

    log_port_list = calloc(MAX_PORTS, sizeof(*log_port_list));
    if (!log_port_list) {
        SX_LOG_ERR("Failed to allocate memory\n");
        return SAI_STATUS_NO_MEMORY;
    }

    status = mlnx_acl_field_port_list_to_sx(port_obj_list, log_port_list, &log_port_count);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to convert port oid to log port\n");
        goto out;
    }

    status = mlnx_acl_sx_mc_container_create_by_log_port(log_port_list, log_port_count, sx_mc_container_id);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to create sx_mc_container\n");
        goto out;
    }
                                                                ...
                                                                _In_ sx_mc_container_id_t    sx_mc_container_id)
{
    ...
    status = mlnx_acl_sx_mc_container_set_impl(SX_ACCESS_CMD_SET, log_port_list, log_port_count,
                                               &sx_mc_container_id);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to set sx_mc_container (%d)\n", sx_mc_container_id);
        return status;
    }
                                                                ...
                                                                _In_ sx_mc_container_id_t     sx_mc_container_id)
{
    ...
    status = mlnx_acl_sx_mc_container_update_by_log_port(log_port_list, log_port_count, sx_mc_container_id);
    if (SAI_ERR(status)) {
...
static sai_status_t mlnx_acl_field_types_check(_In_ mlnx_acl_field_type_t field_type)
{
    ...
    for (ii = 0; ii < mlnx_acl_invalid_field_sets_size; ii++) {
        ...
        if (MLNX_ACL_FIELD_IS_PRESENT(invalid_fields, field_type)) {
            SX_LOG_ERR("Field type combination 0x%x matches invalid combination 0x%x at idx %d - %s\n",
                       field_type,
                       invalid_fields,
                       ii,
 ...
 *
 * The caller is responsible for calling a mlnx_acl_flex_rule_free for flex_acl_rule_p
 */
static sai_status_t mlnx_acl_entry_sx_acl_rule_get(_In_ uint32_t                    acl_table_index,
                                                   _In_ uint32_t                    acl_entry_index,
                                                   _Inout_ sx_flex_acl_flex_rule_t *flex_acl_rule_p)
                                                        ...
                                                        void                          *arg)
{
    ...
    field_info = mlnx_acl_single_key_field_info_fetch(attr_id);
    if (NULL == field_info) {
        SX_LOG_ERR("Failed to fetch info for attr (%d)\n", attr_id);
        SX_LOG_EXIT();
        return SAI_STATUS_FAILURE;
    }
...
{
    sai_status_t status = SAI_STATUS_SUCCESS;
    bool         is_sx_key_present = false;
    uint32_t     sx_key_index = 0;
    uint32_t     ii = 0;
    ...
    for (ii = 0; ii < custom_byte_count; ii++) {
        ...
        if (false == is_sx_key_present) {
            /* If we found that some part of custom bytes are present and some part is not
                                                        ...
                                                        _In_ uint32_t                       custom_byte_count)
{
    ...
    mlnx_acl_flex_rule_key_find(flex_acl_rule, custom_byte_keys[0], &sx_key_index, &is_sx_key_present);
    ...
    data = htons(flex_acl_rule->key_desc_list_p[sx_key_index].key.gp_register);
    mask = htons(flex_acl_rule->key_desc_list_p[sx_key_index].mask.gp_register);

    value->aclfield.enable = true;
    value->aclfield.data.u8list.list[0] = data & 0xFF;
    value->aclfield.mask.u8list.list[0] = mask & 0xFF;

    if (value->aclfield.data.u8list.count > 1) {
        value->aclfield.data.u8list.list[1] = (data >> 8) & 0xFF;
        value->aclfield.mask.u8list.list[1] = (mask >> 8) & 0xFF;
    }
...
{
    sai_status_t            status = SAI_STATUS_SUCCESS;
    sx_acl_key_t            flex_acl_key_ids[SX_FLEX_ACL_MAX_FIELDS_IN_KEY] = {0};
    sx_flex_acl_flex_rule_t flex_acl_rule = MLNX_ACL_SX_FLEX_RULE_EMPTY;
    sai_acl_entry_attr_t    entry_udf_attr = {0};
    const acl_udf_group_t  *udf_group_list = NULL;
    uint32_t                udf_group_length = 0;
    uint32_t                custom_byte_count = 0;
    uint32_t                flex_acl_key_ids_num = 0;
    uint32_t                acl_table_index = 0;
    uint32_t                acl_entry_index = 0;
    uint32_t                acl_table_udf_attr_index = 0;
    uint32_t                udf_group_db_index = 0;
    ...
    status =
        mlnx_udf_group_db_index_to_sx_acl_keys(udf_group_db_index,
                                               flex_acl_key_ids,
    ...
    assert(mlnx_acl_cb);
    status = mlnx_acl_cb->udf_key_descr_to_data
                 (value, &flex_acl_rule, flex_acl_key_ids, custom_byte_count);
    if (SAI_ERR(status)) {
        goto out;
    }
                                                   ...
                                                   _Out_ acl_udf_group_list_t  udf_group_list)
{
    ...
    assert(NULL != flex_acl_key_ids);
    assert(NULL != flex_acl_key_ids_num);
    ...
    for (udf_group_attr_id = SAI_ACL_TABLE_ATTR_USER_DEFINED_FIELD_GROUP_MIN;
        ...
        if (SAI_STATUS_SUCCESS == status) {
            ...
            status = mlnx_udf_group_db_index_to_sx_acl_keys
                         (udf_group_db_index, flex_acl_key_ids, flex_acl_key_ids_num, &custom_bytes_num);
            if (SAI_ERR(status)) {
                return status;
            }
                                                                  ...
                                                                  _Inout_ uint32_t                *key_desc_count)
{
    ...
    for (ii = 0; ii < value->data.u8list.count; ii++) {
        key_desc[*key_desc_count].key_id = flex_acl_key_ids[ii];
                                                                   ...
                                                                   _Inout_ uint32_t                *key_desc_count)
{
    ...
    data = value->data.u8list.list[0];
    mask = value->mask.u8list.list[0];

    if (value->data.u8list.count > 1) {
        data |= value->data.u8list.list[1] << 8;
        mask |= value->mask.u8list.list[1] << 8;
    }

    key_desc[*key_desc_count].key_id = flex_acl_key_ids[0];
    key_desc[*key_desc_count].key.gp_register = ntohs(data);
    key_desc[*key_desc_count].mask.gp_register = ntohs(mask);
                                                   ...
                                                   _Inout_ uint32_t             *key_desc_count)
{
    ...
    uint32_t                     attr_index, custom_bytes_num;
    uint32_t                     flex_acl_key_ids_num = 0;
    ...
    for (udf_attr_id = SAI_ACL_ENTRY_ATTR_USER_DEFINED_FIELD_GROUP_MIN;
        ...
        if (SAI_STATUS_SUCCESS == status) {
            ...
            if (value->enable) {
                ...
                assert(mlnx_acl_cb);
                status = mlnx_acl_cb->udf_data_to_key_descr_list
                             (value, flex_acl_key_ids, custom_bytes_num, key_desc, key_desc_count);
                if (SAI_ERR(status)) {
                    return status;
                }
            }
                                                                ...
                                                                _Inout_ uint32_t                 *custom_byte_count)
{
    ...
    *flex_acl_key_ids_num = 0;
    status =
        mlnx_udf_group_db_index_to_sx_acl_keys
            (udf_group_db_index, flex_acl_key_ids, flex_acl_key_ids_num, custom_byte_count);
    if (SAI_ERR(status)) {
        return status;
    }
                                                        ...
                                                        _In_ uint32_t                     custom_byte_count)
{
    ...
    if (value->aclfield.enable) {
        flex_acl_rule->key_desc_list_p[sx_key_index].key_id = custom_byte_keys[0];

        data = value->aclfield.data.u8list.list[0];
        mask = value->aclfield.mask.u8list.list[0];

        if (custom_byte_count > 1) {
            data |= value->aclfield.data.u8list.list[1] << 8;
            mask |= value->aclfield.mask.u8list.list[1] << 8;
        }

        flex_acl_rule->key_desc_list_p[sx_key_index].key.gp_register = ntohs(data);
        flex_acl_rule->key_desc_list_p[sx_key_index].mask.gp_register = ntohs(mask);

        if (false == is_sx_key_present) {
            flex_acl_rule->key_desc_count++;
        }
    } else if (is_sx_key_present) {
        mlnx_acl_flex_rule_key_del(flex_acl_rule, sx_key_index);
    }
                                           ...
                                           void                             *arg)
{
    ...
    status = mlnx_acl_entry_udf_attrs_validate_and_fetch(value, entry_udf_attr, 0, acl_table_index,
                                                         flex_acl_key_ids, &flex_acl_key_ids_num,
                                                         &custom_bytes_num);
    if (SAI_ERR(status)) {
        goto out;
    }
    ...
    assert(mlnx_acl_cb);
    status = mlnx_acl_cb->udf_data_to_key_descr(value, &flex_acl_rule, flex_acl_key_ids, custom_bytes_num);
    if (SAI_ERR(status)) {
        goto out;
    }
                                              ...
                                              _Inout_ sai_attribute_value_t *value)
{
    ...
    uint16_t            data = 0;
    uint16_t            mask = 0;
    uint32_t            first_sx_key_index = 0;
    uint32_t            second_sx_key_index = 0;
    bool                is_first_sx_key_present = false;
    bool                is_second_sx_key_present = false;
    const sx_acl_key_t *sx_ip_ident_keys = NULL;
    ...
    if (is_first_sx_key_present != is_second_sx_key_present) {
        SX_LOG_ERR("Failed to get IP_IDENTIFICATION failed - sx rule is broken\n");
        status = SAI_STATUS_FAILURE;
        goto out;
    }
                                               ...
                                               _Inout_ sai_attribute_value_t *value)
{
    ...
    value->aclfield.enable = true;
    value->aclfield.data.u16 = flex_acl_rule->key_desc_list_p[first_sx_key_index].key.gp_register;
    value->aclfield.mask.u16 = flex_acl_rule->key_desc_list_p[first_sx_key_index].mask.gp_register;
out:
    return status;
}
                                                ...
                                                void                          *arg)
{
    ...
    assert(mlnx_acl_cb);
    status = mlnx_acl_cb->ip_ident_get(&flex_acl_rule, value);
    if (SAI_ERR(status)) {
        goto out;
    }
                                              ...
                                              _Inout_ sx_flex_acl_flex_rule_t  *flex_acl_rule)
{
    ...
    if (value->aclfield.enable) {
        assert(mlnx_acl_cb);
        status = mlnx_acl_cb->ip_ident_key_descr_add(value, flex_acl_rule->key_desc_list_p,
                                                     &(flex_acl_rule->key_desc_count));
        if (SAI_ERR(status)) {
            goto out;
        }
                                                ...
                                                void                             *arg)
{
    ...
    assert(mlnx_acl_cb);
    status = mlnx_acl_cb->ip_ident_set(value, &flex_acl_rule);
    if (SAI_ERR(status)) {
        goto out;
    }
                                           ...
                                           void                          *arg)
{
    ...
    value->aclfield.enable = false;
    value->aclfield.data.u8 = 0;      /* Initialize the value */
                                             ...
                                             void                             *arg)
{
    ...
    if (new_ports_obj_list) {
        if (is_key_type_present) {
            status = mlnx_acl_sx_mc_container_update_by_port_oid(new_ports_obj_list, mc_container);
            if (SAI_ERR(status)) {
                goto out;
            }
        } else {
            status = mlnx_acl_sx_mc_container_create_by_port_oid(new_ports_obj_list, &mc_container);
            if (SAI_ERR(status)) {
                goto out;
            }
...
/* Different packet action will be set depending on the action and whether trap id bound to the acl
 * If new action is TRAP, COPY or LOG , but current action is different action will be changed to an action that does
 * not require trap id
 * current action not TRAP, COPY or LOG new action TRAP - result action DROP
 * current action not TRAP, COPY or LOG new action LOG - result action FORWARD
 * current action not TRAP, COPY or LOG new action COPY - result action no action
 */
static sai_status_t mlnx_acl_entry_packet_action_set(_In_ const sai_object_key_t      *key,
                                                     _In_ const sai_attribute_value_t *value,
                                                     void                             *arg)
{
    ...
    sx_trap_id_t                   sx_trap = SX_TRAP_ID_ACL_MIN;
    const char                   * trap_name;
    bool                           is_current_action_present, is_action_to_configure_present, is_trap_id_present;
    ...
    is_trap_id_present = is_trap_action_present &&
                         (flex_acl_rule.action_list_p[trap_action_index].fields.action_trap.action ==
                          SX_ACL_TRAP_ACTION_TYPE_TRAP);

    is_current_action_present = is_forward_action_present || is_trap_action_present;

    if (is_current_action_present) {
        status = mlnx_convert_acl_action_to_sai(is_forward_action_present,
                                                flex_acl_rule.action_list_p[forward_action_index].fields.action_forward.action,
                                                is_trap_action_present,
                                                flex_acl_rule.action_list_p[trap_action_index].fields.action_trap.action,
                                                &current_action);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to convert acl SX action to SAI packet action\n");
            goto out;
        }
    }

    is_action_to_configure_present = value->aclaction.enable;

    if (is_action_to_configure_present) {
        action_to_configure = value->aclaction.parameter.s32;

        if ((!is_trap_id_present) && is_action_trap(action_to_configure)) {
            status = mlnx_translate_action_to_no_trap(action_to_configure, &action_to_configure,
                                                      &is_action_to_configure_present);
            if (SAI_ERR(status)) {
                SX_LOG_ERR("Failed to convert action to non-trap action\n");
                goto out;
            }
        }
    }

    if ((!is_current_action_present) && (!is_action_to_configure_present)) {
        goto out;
    }

    if ((is_current_action_present && is_action_to_configure_present) && (current_action == action_to_configure)) {
        goto out;
    }

    if (is_trap_id_present) {
        sx_trap = flex_acl_rule.action_list_p[trap_action_index].fields.action_trap.trap_id;
        status = mlnx_translate_sdk_trap_to_sai(sx_trap, &trap_name, &current_trap);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to get trap id\n");
            goto out;
        }
    }
    ...
    if (is_action_to_configure_present) {
        ...
        status = mlnx_acl_packet_actions_handler(action_to_configure, sx_trap, flex_acl_rule.action_list_p,
                                                 &flex_action_index);
        if (SAI_ERR(status)) {
            goto out;
        }
    ...
    if (is_trap_id_present && ((!is_action_to_configure_present) || (!is_action_trap(action_to_configure)))) {
        status = mlnx_trap_refcount_decrease(current_trap);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to decrease trap refcount\n");
            goto out;
        }
    }
...
static sai_status_t mlnx_acl_flex_rule_update_port_filter(sx_flex_acl_flex_rule_t *flex_acl_rule,
                                                          const sx_port_log_id_t  *port_list,
                                                          const uint32_t           port_count,
                                                          bool                     allow_empty,
                                                          bool                    *rule_changed,
                                                          sx_mc_container_id_t    *containter_to_delete)
{
    sx_flex_acl_flex_action_type_t sx_action_type = SX_FLEX_ACL_ACTION_PORT_FILTER;
    sx_mc_container_id_t           mc_container = SX_MC_CONTAINER_ID_INVALID;
    uint32_t                       sx_action_index;
    bool                           is_action_present;
    sai_status_t                   status;
    ...
    if (is_action_present && (port_count == 0) && (!allow_empty)) {
        *containter_to_delete = mc_container;
    ...
    if ((!is_action_present) && ((port_count != 0) || allow_empty)) {
        status = mlnx_acl_sx_mc_container_create_by_log_port(port_list, port_count, &mc_container);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to create sx_mc_container (%d)\n", mc_container);
            return status;
        }

        sx_action_index = flex_acl_rule->action_count;

        flex_acl_rule->action_list_p[sx_action_index].type = sx_action_type;
        flex_acl_rule->action_list_p[sx_action_index].fields.action_port_filter.mc_container_id = mc_container;
        flex_acl_rule->action_count++;
    ...
    if (is_action_present && ((port_count != 0) || allow_empty)) {
        status = mlnx_acl_sx_mc_container_update_by_log_port(port_list, port_count, mc_container);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to update sx_mc_container (%d)\n", mc_container);
            return status;
        }
...
/* needs sai_db read lock and acl table write lock */
static sai_status_t mlnx_acl_port_isolation_update_impl(_In_ uint32_t                acl_table_index,
                                                        _In_ uint32_t                acl_entry_index,
                                                        _In_ const sx_port_log_id_t *log_port_list,
                                                        _In_ const uint32_t          log_port_count,
                                                        _In_ bool                    allow_empty)
{
    sai_status_t            status;
    sx_flex_acl_flex_rule_t flex_acl_rule = MLNX_ACL_SX_FLEX_RULE_EMPTY;
    ...
    status = mlnx_acl_flex_rule_update_port_filter(&flex_acl_rule, log_port_list, log_port_count, allow_empty,
                                                   &update_sx_rule, &mc_container_to_delete);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to update acl flex rule port filter\n");
        goto out;
    }
    ...
    if (SX_MC_CONTAINER_ID_CHECK_RANGE(mc_container_to_delete)) {
        status = mlnx_acl_sx_mc_container_remove(mc_container_to_delete);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to remove old sx_mc_container (%d)\n", mc_container_to_delete);
            goto out;
...
static sai_status_t mlnx_acl_isolation_group_update_ports(_In_ uint32_t                acl_table_index,
                                                          _In_ uint32_t                acl_entry_index,
                                                          _In_ const sx_port_log_id_t *log_port_list,
                                                          _In_ const uint32_t          log_port_count)
{
    sai_status_t      status;
    sx_port_log_id_t *filter_log_port_list = NULL;
    uint32_t          filter_log_port_cnt = 0;
    bool              allow_empty = false;

    if (log_port_count > 0) {
        filter_log_port_list = (sx_port_log_id_t *)malloc(sizeof(*filter_log_port_list) * MAX_PORTS * 2);
        if (!filter_log_port_list) {
            status = SAI_STATUS_INSUFFICIENT_RESOURCES;
            goto out;
        }
        filter_log_port_cnt = MAX_PORTS * 2;

        status = mlnx_get_switch_log_ports_not_in_lag(log_port_list, log_port_count, filter_log_port_list,
                                                      &filter_log_port_cnt);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to get ports to filter list\n");
            goto out;
        }
    ...
    status = mlnx_acl_port_isolation_update_impl(acl_table_index, acl_entry_index, filter_log_port_list,
                                                 filter_log_port_cnt, allow_empty);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to update port isolation\n");
        goto out;
    }

out:
    free(filter_log_port_list);
                                                        ...
                                                        _In_ const uint32_t          log_port_count)
{
    ...
    status = mlnx_acl_isolation_group_update_ports(acl_table_index, acl_entry_index, log_port_list, log_port_count);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to update port isolation\n");
        return status;
    }
                                                                ...
                                                                void                             *arg)
{
    ...
    if (value->aclaction.enable && (value->aclaction.parameter.objlist.count > 0)) {
        log_port_list = calloc(MAX_PORTS, sizeof(*log_port_list));
        if (!log_port_list) {
            SX_LOG_ERR("Failed to allocate memory\n");
            status = SAI_STATUS_NO_MEMORY;
        ...
        log_port_count = MAX_PORTS;

        status = mlnx_acl_field_port_list_to_sx(&value->aclaction.parameter.objlist, log_port_list, &log_port_count);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to convert port oid to log port\n");
            goto out;
        }
    ...
    status = mlnx_acl_port_isolation_update_impl(acl_table_index, acl_entry_index, log_port_list, log_port_count,
                                                 false);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to update sx port isolation\n");
        goto out;
    }
                                                   ...
                                                   void                             *arg)
{
    ...
    uint32_t                       action_index;
    uint32_t                       table_db_idx, entry_db_idx;
    sx_flex_acl_action_hash_type_t hash_type;
    sx_flex_acl_flex_action_t      action_list[20];
    uint32_t                       action_num;
    uint32_t                       hash_index;
    ...
    assert((action == SAI_ACL_ENTRY_ATTR_ACTION_SET_ECMP_HASH_ID) ||
           (action == SAI_ACL_ENTRY_ATTR_ACTION_SET_LAG_HASH_ID));

    switch (action) {
    case SAI_ACL_ENTRY_ATTR_ACTION_SET_LAG_HASH_ID:
    case SAI_ACL_ENTRY_ATTR_ACTION_SET_ECMP_HASH_ID:
        sx_action = SX_FLEX_ACL_ACTION_HASH;
        break;
    ...
    status = mlnx_acl_entry_sx_acl_rule_get(table_db_idx, entry_db_idx, &flex_acl_rule);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("mlnx_acl_entry_sx_acl_rule_get failed\n");
        goto out;
    }

    mlnx_acl_flex_rule_action_find(&flex_acl_rule, sx_action, &action_index, &is_action_type_present);
    ...
    if (value->aclaction.enable) {
        ...
        hash_type =
            ((int64_t)arg ==
             SAI_ACL_ENTRY_ATTR_ACTION_SET_LAG_HASH_ID) ? SX_ACL_ACTION_HASH_TYPE_LAG : SX_ACL_ACTION_HASH_TYPE_ECMP;
        status = mlnx_fine_grained_hash_create(value->aclaction.parameter.oid, hash_type, action_list, &action_num);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to create fine grained hash ACL actions.\n");
            goto out;
        }
        memcpy(&flex_acl_rule.action_list_p[flex_acl_rule.action_count],
               action_list,
               sizeof(sx_flex_acl_flex_action_t) * action_num);
        flex_acl_rule.action_count += action_num;
    } else {
        action_num = 0;
        if (!is_action_type_present) {
            goto out;
        }
        for (uint32_t index = 0; index < flex_acl_rule.action_count; index++) {
            if ((SX_FLEX_ACL_ACTION_HASH != flex_acl_rule.action_list_p[index].type)
                && (SX_FLEX_ACL_ACTION_ALU_REG != flex_acl_rule.action_list_p[index].type)) {
                action_list[action_num++] = flex_acl_rule.action_list_p[index];
            }
        }
        memcpy(flex_acl_rule.action_list_p, action_list, sizeof(sx_flex_acl_flex_action_t) * action_num);
        flex_acl_rule.action_count = action_num;
    }

    status = mlnx_acl_entry_sx_acl_rule_set(table_db_idx, entry_db_idx, &flex_acl_rule);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to set ACL rule.\n");
        goto out;
    }

    if (SAI_STATUS_SUCCESS ==
        mlnx_object_to_type(value->aclaction.parameter.oid, SAI_OBJECT_TYPE_HASH, &hash_index, NULL)) {
        if (value->aclaction.enable) {
            if (g_sai_db_ptr->hash_list[hash_index].fg_hash_ref_count == 1) {
                if (g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_1_ID] == SAI_NULL_OBJECT_ID) {
                    g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_1_ID] = value->aclaction.parameter.oid;
                } else {
                    g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_2_ID] = value->aclaction.parameter.oid;
                }
            }
        } else {
            ...
            if (g_sai_db_ptr->hash_list[hash_index].fg_hash_ref_count == 0) {
                ...
                if (g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_1_ID] == value->aclaction.parameter.oid) {
                    g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_1_ID] = SAI_NULL_OBJECT_ID;
                } else {
        ...
        if (value->aclaction.enable) {
            acl_db_entry(entry_db_idx).hash_oid = value->aclaction.parameter.oid;
        } else {
            acl_db_entry(entry_db_idx).hash_oid = SAI_NULL_OBJECT_ID;
        }
    }
out:
    SX_LOG_EXIT();
...
static sai_status_t mlnx_acl_ip_ident_key_ref_remove_spc2(void)
{
    ...
    sx_acl_key_t           *sx_keys = sai_acl_db->acl_settings_tbl->ip_ident_keys.sx_keys;
    sx_gp_register_e        reg_id = SX_GP_REGISTER_LAST_E;
    sx_extraction_point_t   ext_point = {0};
    ...
    if (0 == sai_acl_db->acl_settings_tbl->ip_ident_keys.refs) {
        ...
        ext_point.type = SX_EXTRACTION_POINT_TYPE_IPV4_START_OF_HEADER_E;
        ext_point.offset = ACL_IP_IDENT_FIELD_START_OFFSET;

        /* unset extraction point from register key */
        sai_status = mlnx_udf_group_sx_reg_ext_point_set_spc2
                         (SX_ACCESS_CMD_UNSET, reg_id, &ext_point, 1);
                                                           ...
                                                           _Inout_ uint32_t                key_desc_count)
{
    ...
    key_descs[key_desc_count].key_id = ip_ident_keys[0];
    key_descs[key_desc_count].key.gp_register = value;
    key_descs[key_desc_count].mask.gp_register = mask;
                                                              ...
                                                              void                          *arg)
{
    ...
    sai_db_read_lock();
    status = mlnx_get_acl_entry_isolation_group_impl(key->key.object_id, &isolation_group_oid);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to get acl entry isolation group id\n");
        goto out;
    }

    if (isolation_group_oid == SAI_NULL_OBJECT_ID) {
        value->aclaction.enable = false;
        value->aclaction.parameter.oid = SAI_NULL_OBJECT_ID;
    } else {
        value->aclaction.enable = true;
        value->aclaction.parameter.oid = isolation_group_oid;
    }
                                                              ...
                                                              void                             *arg)
{
    ...
    if (value->aclaction.enable) {
        isolation_group_oid = value->aclaction.parameter.oid;
    } else {
        isolation_group_oid = SAI_NULL_OBJECT_ID;
    }
    ...
    status = mlnx_set_acl_entry_isolation_group_impl(key->key.object_id, isolation_group_oid);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to set acl entry isolation group id\n");
        goto out;
    }
...
/*
 * Configured packet action may change when trap id set in following ways
 * 1) Trap id is SAI_NULL_OBJECT_ID or action disabled
 *   a) Current action COPY - result action no action
 *   b) Current action LOG - result action FORWARD
 *   c) Current action TRAP - result action DROP
 * 2) Trap is is not SAI_NULL_OBJECT_ID
 *   a) no current action or action is COPY_CANCEL - result action COPY
 *   b) current action DROP or DENY- result action TRAP
 *   c) current action FORWARD or TRANSIT - result action LOG
 */
static sai_status_t mlnx_acl_entry_action_trap_set(_In_ const sai_object_key_t      *key,
                                                   _In_ const sai_attribute_value_t *value,
                                                   void                             *arg)
{
    ...
    uint8_t             trap_action_index = 0, forward_action_index = 0;
    bool                is_trap_action_present = false, is_forward_action_present = false;
    sx_trap_id_t        sdk_trap;
    sai_object_id_t     user_trap =
        value->aclaction.enable ? value->aclaction.parameter.oid : SAI_NULL_OBJECT_ID;
    uint32_t                       acl_table_index, acl_entry_index;
    sx_flex_acl_flex_rule_t        flex_acl_rule = MLNX_ACL_SX_FLEX_RULE_EMPTY;
    uint32_t                       flex_action_index;
    sx_flex_acl_flex_action_type_t action_type;
    sai_object_id_t                current_trap = SAI_NULL_OBJECT_ID;
    const char                    *trap_name;
    bool                           is_current_action_present, is_action_to_configure_present = true,
                                   is_trap_id_present;
    ...
    status = extract_acl_table_index_and_entry_index(key->key.object_id, &acl_table_index, &acl_entry_index);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to get acl entry 0x%" PRIx64 " table and entry index\n", key->key.object_id);
        return status;
    }
    ...
    if (user_trap != SAI_NULL_OBJECT_ID) {
        if (!mlnx_is_hostif_user_defined_trap_valid_for_set(SAI_OBJECT_TYPE_ACL_ENTRY, user_trap)) {
            SX_LOG_ERR("Invalid trap 0x%" PRIx64 "\n", user_trap);
            status = SAI_STATUS_INVALID_PARAMETER;
            goto out;
        }
    }
    ...
    status = mlnx_acl_entry_sx_acl_rule_get(acl_table_index, acl_entry_index, &flex_acl_rule);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to get acl entry sx rule. Table index - %d, entry index - %d.\n", acl_table_index,
                   acl_entry_index);
        goto out;
    }
    ...
    is_current_action_present = is_forward_action_present || is_trap_action_present;
    is_action_to_configure_present = is_current_action_present;
    ...
    if (is_current_action_present) {
        ...
        if (is_trap_id_present) {
            status = mlnx_translate_sdk_trap_to_sai(
                flex_acl_rule.action_list_p[trap_action_index].fields.action_trap.trap_id,
                &trap_name, &current_trap);
            if (SAI_ERR(status)) {
                SX_LOG_ERR("Failed to get trap id by sx_trap %d\n",
                           flex_acl_rule.action_list_p[trap_action_index].fields.action_trap.trap_id);
                goto out;
            }
        }
    ...
    if (current_trap == user_trap) {
        goto out;
    }

    if (user_trap != SAI_NULL_OBJECT_ID) {
        status = mlnx_get_user_defined_trap_acl_sx_trap_id(user_trap, &sdk_trap);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to get sx trap id for trap 0x%" PRIx64 "\n", user_trap);
            goto out;
        }

        if (!is_trap_id_present) {
            status = mlnx_translate_action_to_trap(is_current_action_present, current_action, &action_to_configure);
            if (SAI_ERR(status)) {
                SX_LOG_ERR("Failed to translate current action to trap action\n");
                goto out;
            }
        }

        is_action_to_configure_present = true;
    } else if (is_trap_id_present) {
        status = mlnx_translate_action_to_no_trap(current_action, &action_to_configure,
                                                  &is_action_to_configure_present);
        if (SAI_ERR(status)) {
    ...
    if (is_action_to_configure_present) {
        ...
        status = mlnx_acl_packet_actions_handler(action_to_configure, sdk_trap, flex_acl_rule.action_list_p,
                                                 &flex_action_index);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to change acl entry action to %d\n", action_to_configure);
            goto out;
        }
        flex_acl_rule.action_count = flex_action_index;
    }
    ...
    if (user_trap != SAI_NULL_OBJECT_ID) {
        status = mlnx_trap_refcount_increase(user_trap);
        if (SAI_ERR(status)) {
                                                   ...
                                                   void                          *arg)
{
    ...
    sx_flex_acl_flex_action_type_t sx_action;
    const char                    *trap_name;
    uint32_t                       acl_table_index, acl_entry_index, action_idx;
    ...
    sx_action = SX_FLEX_ACL_ACTION_TRAP;

    mlnx_acl_flex_rule_action_find(&flex_acl_rule, sx_action, &action_idx, &is_action_type_present);

    if (is_action_type_present &&
        (flex_acl_rule.action_list_p[action_idx].fields.action_trap.action == SX_ACL_TRAP_ACTION_TYPE_TRAP)) {
        status = mlnx_translate_sdk_trap_to_sai(flex_acl_rule.action_list_p[action_idx].fields.action_trap.trap_id,
                                                &trap_name, &value->aclaction.parameter.oid);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to get trap id by sx trap %d\n",
                       flex_acl_rule.action_list_p[action_idx].fields.action_trap.trap_id);
            goto out;
        }
        value->aclaction.enable = true;
    } else {
                                                   ...
                                                   _Inout_ uint32_t                 *sx_action_count)
{
    ...
    assert(ctx->sx_mc_container_port_isolation == SX_MC_CONTAINER_ID_INVALID);
    ...
    status = mlnx_acl_sx_mc_container_create_by_port_oid(&action_data->parameter.objlist, &mc_container);
    if (SAI_ERR(status)) {
        return status;
    }
    ...
    ctx->sx_mc_container_port_isolation = mc_container;
                                               ...
                                               sx_port_log_id_t log_port)
{
    ...
    status = extract_acl_table_index_and_entry_index(acl_entry_id, &acl_table_index, &acl_entry_index);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to extract acl table index and entry index\n");
        return status;
    }
    ...
    if (!is_action_present) {
        SX_LOG_ERR("Failed to update port filter - action is not present\n");
        status = SAI_STATUS_FAILURE;
        goto out;
    }
    mc_container = flex_acl_rule.action_list_p[sx_action_index].fields.action_port_filter.mc_container_id;
    ...
    status = mlnx_acl_sx_mc_container_set_impl(cmd, &log_port, 1, &mc_container);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to set sx_mc_container (%d)\n", mc_container);
        goto out;
    }
                                                  ...
                                                  _Inout_ uint32_t                 *sx_action_count)
{
    ...
    if (action == SAI_ACL_ENTRY_ATTR_ACTION_SET_USER_TRAP_ID) {
        ctx->trap_id = action_data->parameter.oid;
        ctx->is_trap_set = true;
    } else if (action == SAI_ACL_ENTRY_ATTR_ACTION_PACKET_ACTION) {
        ctx->packet_action = action_data->parameter.s32;
        ctx->is_packet_action_set = true;
    }

    if (ctx->is_packet_action_set && (ctx->is_trap_set || (!is_action_trap(ctx->packet_action)))) {
        if (ctx->is_trap_set) {
            status = mlnx_get_user_defined_trap_acl_sx_trap_id(ctx->trap_id, &sx_trap);
            if (SAI_ERR(status)) {
                SX_LOG_ERR("Failed to get sx trap id\n");
                return status;
            }

            status = mlnx_trap_refcount_increase(ctx->trap_id);
            if (SAI_ERR(status)) {
        ...
        status = mlnx_acl_packet_actions_handler(ctx->packet_action, sx_trap, sx_action_list, sx_action_count);
        if (SAI_ERR(status)) {
            return status;
        }
    }
...
static sai_status_t mlnx_convert_acl_action_to_sai(bool                         is_forward_action_present,
                                                   ...
                                                   sai_packet_action_t         *packet_action)
{
    assert(packet_action);

    SX_LOG_ENTER();

    if ((!is_trap_action_present) && (!is_forward_action_present)) {
        SX_LOG_ERR("No action configured\n");
                                                   ...
                                                   _Out_ sai_acl_action_data_t        *action_data)
{
    ...
    sx_flex_acl_forward_action_t forward_action = SX_ACL_TRAP_FORWARD_ACTION_TYPE_FORWARD;
    sx_flex_acl_trap_action_t    trap_action = SX_ACL_TRAP_ACTION_TYPE_TRAP;
    ...
    mlnx_acl_flex_rule_action_find(sx_rule, SX_FLEX_ACL_ACTION_FORWARD, &forward_action_index,
    ...
    status = mlnx_convert_acl_action_to_sai(is_forward_action_present, forward_action, is_trap_action_present,
                                            trap_action, &packet_action);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to convert acl SX action to SAI packet action\n");
        return status;
    }
                                               ...
                                               _Inout_ uint32_t                 *sx_action_count)
{
    ...
    if (SAI_STATUS_SUCCESS !=
        mlnx_object_to_type(action_data->parameter.oid, SAI_OBJECT_TYPE_HASH, &hash_index, NULL)) {
        return SAI_STATUS_FAILURE;
    }
    ...
    hash_type =
        (action ==
         SAI_ACL_ENTRY_ATTR_ACTION_SET_LAG_HASH_ID) ? SX_ACL_ACTION_HASH_TYPE_LAG : SX_ACL_ACTION_HASH_TYPE_ECMP;
    status = mlnx_fine_grained_hash_create(action_data->parameter.oid, hash_type, action_list, &action_num);
    if (SAI_ERR(status)) {
        return SAI_STATUS_FAILURE;
    }
    memcpy(&sx_action_list[*sx_action_count], action_list, sizeof(sx_flex_acl_flex_action_t) * action_num);
    *sx_action_count += action_num;

    if (g_sai_db_ptr->hash_list[hash_index].fg_hash_ref_count == 1) {
        if (g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_1_ID] == SAI_NULL_OBJECT_ID) {
            g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_1_ID] = action_data->parameter.oid;
        } else {
            g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_2_ID] = action_data->parameter.oid;
        }
    }
                                                ...
                                                _Out_ sai_acl_action_data_t        *action_data)
{
    ...
    action_data->enable = true;
    action_data->parameter.oid = entry->hash_oid;
                                                ...
                                                _Inout_ uint32_t                *sx_action_count)
{
    ...
    for (ii = 0; ii < attr_count; ii++) {
        ...
        /*will be applied after acl entry object creation*/
        if (attr_list[ii].id == SAI_ACL_ENTRY_ATTR_ACTION_SET_ISOLATION_GROUP) {
            continue;
        }
                                                 ...
                                                 void                          *arg)
{
    ...
    assert((SAI_ACL_ENTRY_ATTR_ACTION_EGRESS_BLOCK_PORT_LIST == attr_id) ||
           ...
           (SAI_ACL_ENTRY_ATTR_ACTION_SET_LAG_HASH_ID == attr_id) ||
           (SAI_ACL_ENTRY_ATTR_ACTION_SET_ECMP_HASH_ID == attr_id));
...
sai_status_t mlnx_create_acl_entry(_Out_ sai_object_id_t     * acl_entry_id,
                                   ...
                                   _In_ const sai_attribute_t *attr_list)
{
    sai_status_t                 status, trap_status, packet_action_status;
    sx_flex_acl_flex_rule_t      flex_acl_rule = MLNX_ACL_SX_FLEX_RULE_EMPTY;
    ...
    const sai_attribute_value_t *in_port, *in_ports, *out_port, *out_ports, *ip_ident, *hash_id;
    const sai_attribute_value_t *admin_state;
    const sai_attribute_value_t *isolation_group = NULL;
    const sai_attribute_value_t *trap_attr;
    const sai_attribute_value_t *packet_action_attr;
    ...
    uint32_t                     in_port_index, admin_state_index, in_ports_index, ip_ident_index, hash_id_index;
    uint32_t                     out_port_index, out_ports_index;
    uint32_t                     in_port_data, out_port_data;
    uint32_t                     isolation_group_idx;
    uint32_t                     trap_index, packet_action_index;
    ...
    bool                         is_table_dynamic_sized, is_ip_ident_used;
    ...
    packet_action_status = find_attrib_in_list(attr_count,
                                               attr_list,
                                               SAI_ACL_ENTRY_ATTR_ACTION_PACKET_ACTION,
                                               &packet_action_attr,
                                               &packet_action_index);
    if (SAI_OK(packet_action_status)) {
        packet_action = packet_action_attr->aclaction.parameter.s32;
    }

    trap_status = find_attrib_in_list(attr_count, attr_list, SAI_ACL_ENTRY_ATTR_ACTION_SET_USER_TRAP_ID, &trap_attr,
                                      &trap_index);
    if (SAI_OK(trap_status)) {
        trap = trap_attr->aclaction.parameter.oid;
    }

    if (SAI_OK(packet_action_status) && packet_action_attr->aclaction.enable && is_action_trap(packet_action)) {
        if (SAI_ERR(trap_status) || (!trap_attr->aclaction.enable) || (trap == SAI_NULL_OBJECT_ID)) {
            SX_LOG_ERR("Trap packet action requires user trap\n");
            return SAI_STATUS_MANDATORY_ATTRIBUTE_MISSING;
        }
    } else if (SAI_OK(trap_status) && trap_attr->aclaction.enable &&
               (trap != SAI_NULL_OBJECT_ID)) {
        SX_LOG_ERR("Invalid attribute user trap for non-trap packet action\n");
    ...
    is_ip_ident_used = acl_db_table(acl_table_index).is_ip_ident_used;
    sx_direction = mlnx_acl_table_sx_direction_get(&acl_db_table(acl_table_index));
    ...
    if (SAI_STATUS_SUCCESS ==
        ...
        status = mlnx_acl_sx_mc_container_create_by_port_oid(&in_ports->aclfield.data.objlist, &sx_mc_container_rx);
        if (SAI_ERR(status)) {
            goto out;
        }
    ...
    if (SAI_STATUS_SUCCESS ==
        ...
        status = mlnx_acl_sx_mc_container_create_by_port_oid(&out_ports->aclfield.data.objlist, &sx_mc_container_tx);
        if (SAI_ERR(status)) {
            goto out;
        }
    ...
    if (SAI_STATUS_SUCCESS ==
        ...
        assert(mlnx_acl_cb);
        status = mlnx_acl_cb->ip_ident_key_descr_add(ip_ident, sx_key_descs, &key_desc_index);
        if (SAI_ERR(status)) {
            goto out;
        }
    }
    ...
    if (0 == key_desc_index) {
        SX_LOG_ERR(" Mandatory to Send at least one ACL Field during ACL Entry Create \n");
        status = SAI_STATUS_FAILURE;
        goto out;
    }
    ...
    if (g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_1_ID] == SAI_NULL_OBJECT_ID) {
        not_used_hash = SAI_HASH_FG_1_ID;
    } else if (g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_2_ID] == SAI_NULL_OBJECT_ID) {
        not_used_hash = SAI_HASH_FG_2_ID;
    }
    ...
    status = find_attrib_in_list(attr_count,
                                 ...
                                 &isolation_group,
                                 &isolation_group_idx);
    if (!SAI_ERR(status) && isolation_group->aclaction.enable &&
        (isolation_group->aclaction.parameter.oid != SAI_NULL_OBJECT_ID)) {
        status = mlnx_set_acl_entry_isolation_group_impl(*acl_entry_id, isolation_group->aclaction.parameter.oid);
        if (SAI_ERR(status)) {
            goto out;
        }
    }
    ...
    status = SAI_STATUS_SUCCESS;
    if (not_used_hash && g_sai_db_ptr->oper_hash_list[not_used_hash]) {
        acl_db_entry(acl_entry_index).hash_oid = g_sai_db_ptr->oper_hash_list[not_used_hash];
    } else {
        if (SAI_STATUS_SUCCESS ==
            find_attrib_in_list(attr_count, attr_list, SAI_ACL_ENTRY_ATTR_ACTION_SET_ECMP_HASH_ID, &hash_id,
                                &hash_id_index)) {
            acl_db_entry(acl_entry_index).hash_oid = hash_id->aclaction.parameter.oid;
        } else if (SAI_STATUS_SUCCESS ==
                   find_attrib_in_list(attr_count, attr_list, SAI_ACL_ENTRY_ATTR_ACTION_SET_LAG_HASH_ID, &hash_id,
                                       &hash_id_index)) {
            acl_db_entry(acl_entry_index).hash_oid = hash_id->aclaction.parameter.oid;
        }
    }
...
out:
    if (SAI_STATUS_SUCCESS != status) {
        ...
        if (isolation_group && isolation_group->aclaction.enable &&
            (isolation_group->aclaction.parameter.oid != SAI_NULL_OBJECT_ID)) {
            mlnx_set_acl_entry_isolation_group_impl(*acl_entry_id, SAI_NULL_OBJECT_ID);
        }
        ...
        mlnx_acl_sx_mc_container_remove(action_ctx.sx_mc_container_port_isolation);
                                                             ...
                                                             _Inout_ uint32_t     *key_index)
{
    ...
    if (0 == sai_acl_db->acl_settings_tbl->ip_ident_keys.refs) {
        if (g_sai_db_ptr->is_issu_gp_reg_restore) {
            sai_status = mlnx_sai_issu_storage_ip_ident_gp_reg_idx_lookup(&reg_id);
            if (SAI_ERR(sai_status)) {
                SX_LOG_ERR("Failed to restore register info for ip identification\n");
                goto out;
            }

            sai_status = mlnx_gp_reg_db_alloc_by_gp_reg_id(&reg_entry, reg_id);
            if (SAI_ERR(sai_status)) {
                SX_LOG_ERR("Failed to allocate register for ip identification\n");
                goto out;
            }
        } else {
            sai_status = mlnx_gp_reg_db_alloc_first_free(&reg_entry, &gp_reg_db_idx, GP_REG_USED_IP_IDENT);
            if (SAI_ERR(sai_status)) {
                SX_LOG_ERR("Failed to find free register for ip identification\n");
                goto out;
            }
        ...
        reg_entry->gp_usage = GP_REG_USED_IP_IDENT;

        ext_point.type = SX_EXTRACTION_POINT_TYPE_IPV4_START_OF_HEADER_E;
        ext_point.offset = ACL_IP_IDENT_FIELD_START_OFFSET;

        /* set extraction point to register key in SDK */
        if (SAI_STATUS_SUCCESS !=
            (sai_status = mlnx_udf_group_sx_reg_ext_point_set_spc2
                              (SX_ACCESS_CMD_SET, reg_id, &ext_point, 1))) {
            goto out;
        }

        /* allocate FLEX ACL key id*/
        sai_acl_db->acl_settings_tbl->ip_ident_keys.sx_keys[0] = MLNX_SX_GP_REG_TO_FLEX_ACL_KEY(reg_id);
    }

    sai_acl_db->acl_settings_tbl->ip_ident_keys.refs++;

    keys[*key_index] = sai_acl_db->acl_settings_tbl->ip_ident_keys.sx_keys[0];
                                   ...
                                   _In_ const sai_attribute_t *attr_list)
{
    ...
    if (SAI_STATUS_SUCCESS == status) {
        if (true == ip_ident->booldata) {
            assert(mlnx_acl_cb);
            status = mlnx_acl_cb->ip_ident_key_create_or_get(keys, &key_index);
            if (SAI_ERR(status)) {
                goto out;
            }
...
out:
    if (status != SAI_STATUS_SUCCESS) {
        ...
        if (is_ip_ident_used) {
            assert(mlnx_acl_cb);
            mlnx_acl_cb->ip_ident_key_ref_remove();
        }
    }
...
static void acl_group_member_key_to_str(_In_ sai_object_id_t acl_group_memeber_id, _Out_ char *key_str)
{
    ...
    if (SAI_STATUS_SUCCESS != mlnx_object_to_type(acl_group_memeber_id, SAI_OBJECT_TYPE_ACL_TABLE_GROUP_MEMBER,
        ...
        snprintf(key_str, MAX_KEY_STR_LEN, "ACL Group member [%u]", group_member_id);
    }
}
...
static sai_status_t mlnx_delete_acl_entry(_In_ sai_object_id_t acl_entry_id)
{
    ...
    sai_object_id_t fg_hash_oid = SAI_NULL_OBJECT_ID;
    ...
    fg_hash_oid = acl_db_entry(acl_entry_index).hash_oid;
    ...
    status = mlnx_set_acl_entry_isolation_group_impl(acl_entry_id, SAI_NULL_OBJECT_ID);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to unset isolation group\n");
        goto out;
    }
    ...
    if (fg_hash_oid != SAI_NULL_OBJECT_ID) {
        if (SAI_STATUS_SUCCESS !=
            (status = mlnx_object_to_type(fg_hash_oid, SAI_OBJECT_TYPE_HASH, &hash_data, NULL))) {
            goto out;
        }
        g_sai_db_ptr->hash_list[hash_data].fg_hash_ref_count--;

        if (g_sai_db_ptr->hash_list[hash_data].fg_hash_ref_count == 0) {
            gp_registers_delete(g_sai_db_ptr->hash_list[hash_data].fg_fields);
            if (g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_1_ID] == fg_hash_oid) {
                g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_1_ID] = SAI_NULL_OBJECT_ID;
            } else {
                g_sai_db_ptr->oper_hash_list[SAI_HASH_FG_2_ID] = SAI_NULL_OBJECT_ID;
            }
...
static sai_status_t mlnx_delete_acl_table(_In_ sai_object_id_t acl_table_id)
{
    ...
    if (is_ip_ident_used) {
        assert(mlnx_acl_cb);
        status = mlnx_acl_cb->ip_ident_key_ref_remove();
        if (SAI_ERR(status)) {
            goto out;
        }
    }
...
/*
 * It is safe to call this function if rule was initialized with MLNX_ACL_SX_FLEX_RULE_EMPTY value
 */
static sai_status_t mlnx_acl_flex_rule_free(_In_ sx_flex_acl_flex_rule_t *rule)
{
    sx_status_t sx_status;
...
static sai_status_t mlnx_acl_sx_rule_mc_containers_remove(_In_ sx_flex_acl_flex_rule_t *sx_rule)
{
    ...
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to destroy RX sx_mc_container %d\n", rx_list);
        return status;
    }

    status = mlnx_acl_sx_mc_container_remove(tx_list);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to destroy TX sx_mc_container %d\n", tx_list);
        return status;
    }

    status = mlnx_acl_sx_mc_container_remove(egr_block);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to destroy Egress block sx_mc_container %d\n", egr_block);
        return status;
    }
...
static sai_status_t mlnx_remove_acl_trap(uint32_t acl_entry_index, uint32_t acl_table_index)
{
    sai_status_t                   status;
    ...
    status = mlnx_acl_entry_sx_acl_rule_get(acl_table_index, acl_entry_index, &flex_acl_rule);
    if (SAI_ERR(status)) {
        SX_LOG_ERR("Failed to get sx ACL entry rule\n");
        return status;
    }
    ...
    if (is_action_type_present &&
        ...
        status = mlnx_translate_sdk_trap_to_sai(flex_acl_rule.action_list_p[action_idx].fields.action_trap.trap_id,
                                                &trap_name, &trap_id);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to get sdk trap id\n");
            goto out;
        }

        status = mlnx_trap_refcount_decrease(trap_id);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to decrease trap refcount\n");
            goto out;
        }
...
static sai_status_t mlnx_delete_acl_entry_data(_In_ uint32_t table_index, _In_ uint32_t entry_index)
{
    ...
    status = mlnx_remove_acl_trap(entry_index, table_index);
    if (SAI_ERR(status)) {
        goto out;
    }
...
static sai_status_t mlnx_acl_rpc_call(_Inout_ acl_rpc_info_t *rpc_info)
{
    ...
    if (bytes != sizeof(*rpc_info)) {
        SX_LOG_ERR("Failed to send data through the socket - %s\n", strerror(errno));
        status = SAI_STATUS_FAILURE;
        goto out;
    }
...
static sai_status_t mlnx_acl_table_init_sp2(_In_ uint32_t table_db_idx, _In_ bool is_table_dynamic, _In_ uint32_t size)
{
    ...
    if (table->is_inited) {
        SX_LOG_ERR("Table %d in acl_sp2_table_db is already initialized\n", table_db_idx);
        return SAI_STATUS_FAILURE;
    }
...
static sai_status_t mlnx_acl_table_deinit_sp2(_In_ uint32_t table_db_idx)
{
    ...
    if (!table->is_inited) {
        SX_LOG_ERR("Table %u in acl_sp2_table_db is not initialized\n", table_db_idx);
        return SAI_STATUS_FAILURE;
    }
                                                  ...
                                                  _In_ sx_acl_rule_offset_t offset)
{
    ...
    SX_LOG_DBG("ACL table idx %d: deinitialized entry offset %u, allocated - %u, table size = %u\n",
               table_db_idx,
               offset,
               table->allocated,
               table->size);
...
static sai_status_t mlnx_acl_init_sp(void)
{
    ...
    if (0 != pthread_condattr_destroy(&cond_attr)) {
        SX_LOG_ERR("Failed to destroy condition variable attribute for ACL\n");
        return SAI_STATUS_FAILURE;
    }
#endif /* _WIN32 */
...
sai_status_t mlnx_acl_init(void)
{
    ...
    if (0 != pthread_mutexattr_destroy(&mutex_attr)) {
        SX_LOG_ERR("Failed to destroy mutex attribute for ACL\n");
        return SAI_STATUS_FAILURE;
    }
                                                   ...
                                                   _In_ bool                      is_acl_db_locked)
{
    ...
    sx_access_cmd_t    sx_port_isolation_cmd;
    sx_port_log_id_t   sx_port_id;
    ...
    case ACL_EVENT_TYPE_LAG_MEMBER_DEL:
        ...
        sx_port_isolation_cmd = SX_ACCESS_CMD_ADD;
    ...
    case ACL_EVENT_TYPE_LAG_MEMBER_ADD:
        ...
        sx_port_isolation_cmd = SX_ACCESS_CMD_DELETE;
    ...
    if (is_isolation_group_in_use()) {
        status = mlnx_isolation_group_update_mc_containers(sx_port_isolation_cmd, sx_port_id);
        if (SAI_ERR(status)) {
            SX_LOG_ERR("Failed to update isolation group acl entry mc_container\n");
            goto out;
        }
    }
                                                        ...
                                                        _Inout_ uint32_t                 *key_desc_index)
{
    ...
    assert(ip_ident);
    assert(sx_key_descs);
    assert(key_desc_index);
    assert(mlnx_acl_cb);

    sai_status = mlnx_acl_cb->ip_ident_key_descr_create(ip_ident->aclfield.data.u16, ip_ident->aclfield.mask.u16,
                                                   ...
                                                   _Inout_ uint32_t             *sx_port_count)
{
    ...
    assert(port_list);
    assert(sx_port_list);
    assert(sx_port_count);
    assert(*sx_port_count >= port_list->count);

    for (ii = 0; ii < port_list->count; ii++) {
        status = mlnx_acl_field_port_to_sx(port_list->list[ii], &sx_port_list[ii]);
        if (SAI_ERR(status)) {
            return status;
        }
    }

    *sx_port_count = port_list->count;
                                                      ...
                                                      _In_ uint32_t                   table_index)
{
    ...
    sai_acl_range_type_t        range_list_types[ACL_RANGE_MAX_COUNT] = { 0 };
...
sai_status_t mlnx_gp_reg_db_alloc(_Out_ mlnx_gp_reg_db_t **gp_reg_data, _Out_ mlnx_shm_rm_array_idx_t  *idx)
{
    ...
    status = mlnx_shm_rm_array_alloc(MLNX_SHM_RM_ARRAY_TYPE_GP_REG, idx, &ptr);
    if (SAI_ERR(status)) {
        return status;
    }
...
sai_status_t mlnx_gp_reg_db_alloc_by_gp_reg_id(_Out_ mlnx_gp_reg_db_t **gp_reg_data, sx_gp_register_e reg_id)
{
    ...
    idx.idx = reg_id;
    idx.type = MLNX_SHM_RM_ARRAY_TYPE_GP_REG;

    sai_status = mlnx_shm_rm_idx_validate(idx);
    if (SAI_ERR(sai_status)) {
        goto out;
    }

    sai_status = mlnx_gp_reg_db_idx_to_data(idx, gp_reg_data);
    if (SAI_ERR(sai_status)) {
        goto out;
    }

    if ((*gp_reg_data)->mlnx_array.is_used) {
        sai_status = SAI_STATUS_ITEM_ALREADY_EXISTS;
        goto out;
    }

    (*gp_reg_data)->mlnx_array.is_used = true;
                                             ...
                                             _In_ mlnx_gp_reg_usage_t       reg_usage)
{
    ...
    /* Lookup for unallocated gp register starts from the beginning for PBH objects
     * and from the other end of db for the rest of objects */
    switch (reg_usage) {
    ...
    case GP_REG_USED_UDF:
        step = -1;
        /*
         * UDF object can be used for ACL purposes or for ECMP hash purposes.
         * Several last gp resisters do not support hash operations.
         * It is not possible to determine UDF object usage purpose on creation.
         * Thus, to avoid complexity in allocation logic, number of gp registers
         * accessible for usage by UDF object is limited.
         * Lookup for unallocated gp register for UDF object starts from gp
         * register appropriate for both usage purposes.
         * SX enum values are used to determine last GP register which has
         * hash operations capability.
         * Two custom bytes are mapped to one gp register.
         */
        reg_id = (SX_ROUTER_ECMP_HASH_GENERAL_FIELDS_GP_REGISTER_LAST -
                  SX_ROUTER_ECMP_HASH_GENERAL_FIELDS_GP_REGISTER_0_BYTE_0) / 2;
        break;

    case GP_REG_USED_IP_IDENT:
        step = -1;
        /* IP identification is configured using ACL. Thus any gp register can be used */
        reg_id = MLNX_UDF_GP_REG_COUNT - 1;
        break;
    ...
    for (; reg_id >= 0 && reg_id < (int)MLNX_UDF_GP_REG_COUNT; reg_id += step) {
        sai_status = mlnx_gp_reg_db_alloc_by_gp_reg_id(gp_reg_data, reg_id);
        if (SAI_STATUS_ITEM_ALREADY_EXISTS == sai_status) {
...
sai_status_t mlnx_gp_reg_db_free(_In_ mlnx_shm_rm_array_idx_t idx)
{
    ...
    status = mlnx_gp_reg_db_idx_to_data(idx, &gp_reg_db_data);
    if (SAI_ERR(status)) {
        return status;
    }
