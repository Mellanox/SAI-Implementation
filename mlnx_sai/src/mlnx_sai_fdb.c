                                      ...
                                      void                             *arg)
{
    ...
    if ((SAI_FDB_ENTRY_DYNAMIC == value->s32) && (SX_FDB_ACTION_FORWARD != old_mac_entry.action)) {
                                      ...
                                      void                             *arg)
{
    ...
    if (SAI_NULL_OBJECT_ID == value->oid) {
        ...
        mlnx_fdb_route_action_fetch(SAI_OBJECT_TYPE_FDB, fdb_entry, &new_mac_entry);
                                        ...
                                        void                             *arg)
{
    ...
    if ((SX_FDB_IS_PORT_REDUNDANT(old_mac_entry.entry_type, old_mac_entry.action)) &&
        ...
        status = mlnx_fdb_route_action_save(SAI_OBJECT_TYPE_FDB, fdb_entry, value->s32);
        if (SAI_ERR(status)) {
            return status;
        }
    } else {
        ...
        mlnx_fdb_route_action_clear(SAI_OBJECT_TYPE_FDB, fdb_entry);
    }
